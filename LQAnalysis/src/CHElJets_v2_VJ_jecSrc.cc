...
/***************************************************************************
 * @Project: LQCHElJets_v2_VJ Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
#include "CHElJets_v2_VJ_jecSrc.h"
...
ClassImp (CHElJets_v2_VJ_jecSrc);

/**
 *   This is an Example Cycle. It inherits from AnalyzerCore. The code contains all the base class functions to run the analysis.
 *
 */
CHElJets_v2_VJ_jecSrc::CHElJets_v2_VJ_jecSrc() :  AnalyzerCore(),  out_electrons(0) {

	// To have the correct name in the log:
 
	SetLogName("CHElJets_v2_VJ_jecSrc");
	Message("In CHElJets_v2_VJ_jecSrc constructor", INFO);
	//
...
void CHElJets_v2_VJ_jecSrc::InitialiseAnalysis() throw( LQError ) {
...
void CHElJets_v2_VJ_jecSrc::ExecuteEvents()throw( LQError ){
...
int CHElJets_v2_VJ_jecSrc::ExecuteSystematics(int method, TString label)throw( LQError ){
...
//	int event_number = eventbase->GetEvent().EventNumber();
	...
	*/	  
	...
	if (!(muonTightColl.size()==0 && electronTightColl.size()==1)) throw LQError( "Fails 1 Tight muon",  LQError::SkipEvent );
	...
	for(unsigned int ij=0; ij < jetColl_lepveto.size(); ij++){
	  ...
	  else if(method == 2) jet_pt = jetColl_lepveto.at(this_idx).ScaledDownPt();
	  else if(method == 3) jet_pt = jetColl_lepveto.at(this_idx).SmearedUpPt(); 
	  else if(method == 4) jet_pt = jetColl_lepveto.at(this_idx).SmearedDownPt(); 
	  else if (method ==5 && (jetFlavour == 5)){// B-up
	  ...
	  else if (method == 6 && jetFlavour == 5){ // B-down
	  ...
	  else if (method==7 && jetFlavour != 5){// UDSCG-up
	...
	if (!k_isdata)	fBTagSF->GetBTagEvtWeight(njets,jetPt_all,jetEta_all,jetFlav_all,btag_all,0,0,Bweight,BweightErr,VJ);
	
	weight *= Bweight;
	if (!k_isdata)	fBTagSF->GetBTagEvtWeight(njets,jetPt_all,jetEta_all,jetFlav_all,btag_all,1,0,Bweight_up,BweightErr,VJ);
	if (!k_isdata)	fBTagSF->GetBTagEvtWeight(njets,jetPt_all,jetEta_all,jetFlav_all,btag_all,-1,0,Bweight_down,BweightErr,VJ);
	if (!k_isdata)	fBTagSF->GetBTagEvtWeight(njets,jetPt_all,jetEta_all,jetFlav_all,btag_all,0,1,Misweight_up,BweightErr,VJ);
	if (!k_isdata)	fBTagSF->GetBTagEvtWeight(njets,jetPt_all,jetEta_all,jetFlav_all,btag_all,0,-1,Misweight_down,BweightErr,VJ);
...
void CHElJets_v2_VJ_jecSrc::EndCycle()throw( LQError ){
  
	Message("In EndCycle" , INFO);
}


void CHElJets_v2_VJ_jecSrc::BeginCycle() throw( LQError ){
...
CHElJets_v2_VJ_jecSrc::~CHElJets_v2_VJ_jecSrc() {
  
	Message("In CHElJets_v2_VJ_jecSrc Destructor" , INFO);
...
void CHElJets_v2_VJ_jecSrc::FillCutFlow(TString label,TString cut, float weight){
...
void CHElJets_v2_VJ_jecSrc::BeginEvent( )throw( LQError ){
...
void CHElJets_v2_VJ_jecSrc::MakeHistograms(TString label){
  ...
  Message("Made histograms", INFO);
	/**
	 *  Remove//Overide this CHElJets_v2_VJ_jecSrcCore::MakeHistograms() to make new hists for your analysis
	 **/
...
void CHElJets_v2_VJ_jecSrc::ClearOutputVectors() throw(LQError) {
...
void CHElJets_v2_VJ_jecSrc::mysort(std::vector<snu::KJet> jetColl_lepveto, int jet_index[]){
