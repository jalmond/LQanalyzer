...
void ExampleAnalyzerDiMuon::ExecuteEvents()throw( LQError ){
  ...
  if (muonTightColl.size() == 2){ 
    if(abs(muonTightColl.at(0).Pt() - 48.) < 10.) ptrange=true;
    if(abs(muonTightColl.at(1).Pt() - 48.) < 10.) ptrange=true;
  }
  
  if(muonTightColl.size()==2){  
    FillHist("muon1_pt", muonTightColl.at(0).Pt(), weight*pileup_reweight, 0., 200.,200);
    FillHist("muon2_pt", muonTightColl.at(1).Pt(), weight*pileup_reweight, 0., 200.,200);
    if(ptrange)     FillHist("muon_ptrange_pt", muonTightColl.at(1).Pt(), weight*pileup_reweight, 0., 200.,200);
    else FillHist("muon_fail_ptrange_pt", muonTightColl.at(1).Pt(), weight*pileup_reweight, 0., 200.,200);
    FillHist("njets", jetColl_lepveto.size(), weight*pileup_reweight, 0., 5.,5);

  }
 
  if (muonLooseColl.size() ==2  && muonTightColl.size() == 2 && jetColl_lepveto.size()==0 && eventbase->GetEvent().PFMET() < 54. && ptrange) {                   

     snu::KParticle Z = muonTightColl.at(0) + muonTightColl.at(1);
     if(Z.M() > 20.){
       if(muonTightColl.at(0).Charge() != muonTightColl.at(1).Charge()){      
	 FillCutFlow("DiMu_tight", pileup_reweight*weight);
	 
	 /// Method of plotting single histogram
	 FillHist("zpeak_mumu", Z.M(), weight*pileup_reweight, 0., 200.,400);
