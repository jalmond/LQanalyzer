...
/***************************************************************************
 * @Project: LQCHMuJets_v3_sig Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
#include "CHMuJets_v3_sig_jecFlav.h"
...
ClassImp (CHMuJets_v3_sig_jecFlav);

/**
 *   This is an Example Cycle. It inherits from AnalyzerCore. The code contains all the base class functions to run the analysis.
 *
 */
CHMuJets_v3_sig_jecFlav::CHMuJets_v3_sig_jecFlav() :  AnalyzerCore(),  out_electrons(0) {

  // To have the correct name in the log:
 
  SetLogName("CHMuJets_v3_sig_jecFlav");
  Message("In CHMuJets_v3_sig_jecFlav constructor", INFO);
  //
...
void CHMuJets_v3_sig_jecFlav::InitialiseAnalysis() throw( LQError ) {
  
  /// Initialise histograms
  //	MakeHistograms("");  
  MakeHistograms("L5Bup");  
  MakeHistograms("L5Bdown");  
  MakeHistograms("L5UDSCGup");  
  MakeHistograms("L5UDSCGdown");  
  //
...
void CHMuJets_v3_sig_jecFlav::ExecuteEvents()throw( LQError ){

  float default_weight = weight;
  int keep_event= ExecuteSystematics(5, "L5Bup");
  weight = default_weight;
  keep_event+=    ExecuteSystematics(6, "L5Bdown");
  weight=default_weight;
  keep_event+=    ExecuteSystematics(7, "L5UDSCGup");
  weight=default_weight;
  keep_event+=    ExecuteSystematics(8, "L5UDSCGdown");

  if(keep_event==0)throw LQError( "Fails basic cuts",  LQError::SkipEvent ); 
}

int CHMuJets_v3_sig_jecFlav::ExecuteSystematics(int method, TString label)throw( LQError ){
  ...
  for (int i=0; i<(int)genBColl.size()-1; i++){
   ...
   int mother = 	  genBColl.at(i).IndexMother();
    ...
    if (abs(genBColl.at(i).PdgId()) > 10 && abs(genBColl.at(i).PdgId()) < 20  && genBColl.at(mother).GenStatus() == 3 && (abs( genBColl.at(mother).PdgId() ) == 24) ){
      dilep++;
      if (debug)      cout << " Checked dilep = " << dilep << endl;
    }
    ...
    if (debug) 
      cout << " consider checks : " << dilep << ", " << found_top << ", " << found_atop << ", " << foundQ1 << ", " << foundQ2 << endl;
...
void CHMuJets_v3_sig_jecFlav::EndCycle()throw( LQError ){
  
  Message("In EndCycle" , INFO);
}


void CHMuJets_v3_sig_jecFlav::BeginCycle() throw( LQError ){
...
CHMuJets_v3_sig_jecFlav::~CHMuJets_v3_sig_jecFlav() {
  
  Message("In CHMuJets_v3_sig_jecFlav Destructor" , INFO);
  if(!k_isdata)delete reweightPU;
  if(!k_isdata) delete fBTagSF;
  //	delete minuit;
}
         
void CHMuJets_v3_sig_jecFlav::FillCutFlow(TString label,TString cut, float weight){
...
void CHMuJets_v3_sig_jecFlav::BeginEvent( )throw( LQError ){
...
void CHMuJets_v3_sig_jecFlav::ClearOutputVectors() throw(LQError) {
...
void CHMuJets_v3_sig_jecFlav::mysort(std::vector<snu::KJet> jetColl_lepveto, int jet_index[]){
