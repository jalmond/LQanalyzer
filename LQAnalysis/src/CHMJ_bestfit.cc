...
void CHMJ_bestfit::ExecuteEvents()throw( LQError ){
	...
	if (!k_isdata){
	  ...
	  //	fBTagSF->GetBTagEvtWeight(njets,jetPt_f,jetEta_f,jetFlav_f,btag_f,0,Bweight,BweightErr,QCD);
		/*	
		float bw_up = 1.0, bw_down = 1.0;
		fBTagSF->GetBTagEvtWeight(4,jetPt_f,jetEta_f,jetFlav_f,btag_f,-1,bw_down,BweightErr,QCD);
		fBTagSF->GetBTagEvtWeight(4,jetPt_f,jetEta_f,jetFlav_f,btag_f,1,bw_up,BweightErr,QCD);
	...
	/*
		...
		in_list[12] = (jin_atb+Mu_vec+met).M();  // lep top mass
		...
		fitterStudy[36] = Mu_vec.Pt();
		fitterStudy[37] = Mu_vec.Eta();
		fitterStudy[38] = Mu_vec.Phi();
