...
/***************************************************************************
 * @Project: LQCHEJ_bestfit Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
ClassImp (CHEJ_bestfit);

/**
 *   This is an Example Cycle. It inherits from AnalyzerCore. The code contains all the base class functions to run the analysis.
 *
 */
CHEJ_bestfit::CHEJ_bestfit() :  AnalyzerCore(),  out_electrons(0) {

	// To have the correct name in the log:
	
	SetLogName("CHEJ_bestfit");

	Message("In CHEJ_bestfit constructor", INFO);
	//
...
void CHEJ_bestfit::InitialiseAnalysis() throw( LQError ) {
...
void CHEJ_bestfit::ExecuteEvents()throw( LQError ){
	...
	if (debug){
		...
		cout << " EL size = " << electronTightColl.size()<< endl;
	}

	FillCutFlow("TriggerCut",weight);
	if (debug) cout << " Trigger SF applied " << endl;

	
	if (electronTightColl.size()!=1) throw LQError( "Fails Tight Electron == 1 cuts",  LQError::SkipEvent );
	if (debug) cout << " Pass EL cut " << endl;
	...
	if (!k_isdata){
		float temp_el = TopElIDIsoScaleFactor(El.Pt(), El.Eta(), 0);
		weight *= temp_el;
		TopElIDIsoSF_up_syst_factor = TopElIDIsoScaleFactor(El.Pt(), El.Eta(), 1)/temp_el;
		TopElIDIsoSF_down_syst_factor = TopElIDIsoScaleFactor(El.Pt(), El.Eta(), -1)/temp_el;
	}

	
	FillCutFlow("OneTightEl",weight);


	
	if (electronVetoColl.size() >1) throw LQError( "Fails electronLoose cuts",  LQError::SkipEvent );
		
	FillCutFlow("ExtraElVeto",weight);

	if (muonVetoColl.size()>0)  throw LQError( "Fails muonLoose cuts",  LQError::SkipEvent );	

	FillCutFlow("MuVeto",weight);
	...
	if (debug) cout << " Number of tagged jets = " << nbtags_tot << ", in four jets: " << nbtags_f << endl;
	...
	float Bw_up_syst_factor = 1.0, Bw_down_syst_factor = 1.0;
	
	// Consider only leading four jets for b-tagging
	if (debug) cout << "  Applying btag SF " << endl;	
	...
	FillHist("h_el_pt_met",El.Pt(),weight);
	...
	if (nbtags_f == 2){
		...
		FillHist("in_lep_2b",El.Pt(),weight);
		FillHist("in_lepeta_2b",El.Eta(),weight);
		...
		FillHist("in_lep_3b",El.Pt(),weight);
		FillHist("in_lepeta_3b",El.Eta(),weight);
	...
	/*
		...
		//myfit->Fit(El_vec,sigmalep,jets_f,jetraw_f,btag_f,sumjet_uncorr,met,0, ntype, debug, highpt_fit);
		...
		in_list[10] = high_mjj;
		in_list[11] = (jin_tb+jin_q1+jin_q2).M(); // had top mass
		in_list[12] = (jin_atb+El_vec+met).M();  // lep top mass
		in_list[13] = (fit_q1+fit_q2).M(); // fit result
		in_list[14] = weight;  
		FillNtp("in",in_list);
		
		if (fitcorr){
		  FillHist("h_jettbm_corr",raw_tb.M(),weight);
		  FillHist("h_jetatbm_corr",raw_atb.M(),weight);
		  FillHist("h_jetq1m_corr",raw_q1.M(),weight);
		  FillHist("h_jetq2m_corr",raw_q2.M(),weight);

		  FillHist("h_jettbptdiff_corr",fabs(fit_tb.Pt()-jin_tb.Pt()),weight);
		  FillHist("h_jetatbptdiff_corr",fabs(fit_atb.Pt()-jin_atb.Pt()),weight);
		  FillHist("h_jetq1ptdiff_corr",fabs(fit_q1.Pt()-jin_q1.Pt()),weight);
		  FillHist("h_jetq2ptdiff_corr",fabs(fit_q2.Pt()-jin_q2.Pt()),weight);
		}
		else{
		  FillHist("h_jettbm_wrong",raw_tb.M(),weight);
		  FillHist("h_jetatbm_wrong",raw_atb.M(),weight);
		  FillHist("h_jetq1m_wrong",raw_q1.M(),weight);
		  FillHist("h_jetq2m_wrong",raw_q2.M(),weight);

		  FillHist("h_jettbptdiff_wrong",fabs(fit_tb.Pt()-jin_tb.Pt()),weight);
		  FillHist("h_jetatbptdiff_wrong",fabs(fit_atb.Pt()-jin_atb.Pt()),weight);
		  FillHist("h_jetq1ptdiff_wrong",fabs(fit_q1.Pt()-jin_q1.Pt()),weight);
		  FillHist("h_jetq2ptdiff_wrong",fabs(fit_q2.Pt()-jin_q2.Pt()),weight);
		}
		...
		fitterStudy[41] = genallj_match;		
		fitterStudy[42] = weight;
		fitterStudy[43] = mHiggs;
...
void CHEJ_bestfit::EndCycle()throw( LQError ){
	
	Message("In EndCycle" , INFO);
}


void CHEJ_bestfit::BeginCycle() throw( LQError ){
...
CHEJ_bestfit::~CHEJ_bestfit() {
  
	Message("In CHEJ_bestfit Destructor" , INFO);
...
void CHEJ_bestfit::FillCutFlow(TString cut, float weight){
	...
	if(GetHist("cutflow")) {
		...
		GetHist("cutflow")->GetXaxis()->SetBinLabel(7,"OneTightEl");
		GetHist("cutflow")->GetXaxis()->SetBinLabel(8,"ExtraElVeto");
		GetHist("cutflow")->GetXaxis()->SetBinLabel(9,"MuVeto");		
...
void CHEJ_bestfit::BeginEvent( )throw( LQError ){
...
void CHEJ_bestfit::MakeHistograms(){
	...
	maphist["h_el_pt_4jets"] = new TH1F("h_el_pt_4jets","Electron Pt, 4jets",40,0,200);
	...
	maphist["h_el_pt_2b"] = new TH1F("h_el_pt_2b","Electron Pt, 2btag",40,0,200);
	...
	maphist["h_el_pt_3b"] = new TH1F("h_el_pt_3btag","Electron Pt, 3btag",40,0,200);
	...
	maphist["h_el_pt_2b_f"] = new TH1F("h_el_pt_2b_f","Electron Pt, 4j 2btag",40,0,200);
	...
	maphist["h_el_pt_3b_f"] = new TH1F("h_el_pt_3b_f","Electron Pt, 4j 3btag",40,0,200);
	...
	maphist["BW_el_pt_2b"] = new TH1F("BW_el_pt_2b","Electron Pt, 2btag",40,0,200);
	...
	maphist["BW_el_pt_3b"] = new TH1F("BW_el_pt_3b","Electron Pt, 3btag",40,0,200);
	...
	maphist["BW_el_pt_2b_f"] = new TH1F("BW_el_pt_2b_f","Electron Pt, 4j 2btag",40,0,200);
	...
	maphist["BW_el_pt_3b_f"] = new TH1F("BW_el_pt_3b_f","Electron Pt, 4j 3btag",40,0,200);
	...
	maphist["h_el_pt_met"] = new TH1F("h_el_pt_met","Electron Pt, metcut",40,0,200);
	...
	/**
	 *  Remove//Overide this CHEJ_bestfitCore::MakeHistograms() to make new hists for your analysis
	 **/
	/*
...
void CHEJ_bestfit::ClearOutputVectors() throw(LQError) {
...
void CHEJ_bestfit::mysort(std::vector<snu::KJet> jetColl_lepveto, int jet_index[]){
...
void CHEJ_bestfit::Sortout(float in1, float in2, float in3, float& out1, float& out2, float& out3){
