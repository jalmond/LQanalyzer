/***************************************************************************
 * @Project: LQAnalyzer Frame - ROOT-based analysis framework for Korea LQ
 * @Package: LQCycles
 *
...
AnalyzerCore::AnalyzerCore() : LQCycleBase(), n_cutflowcuts(0), MCweight(-999.),reset_lumi_mask(false),changed_target_lumi(false), k_reset_period(false), a_mcperiod(-1),comp_file_firstev(true) {
  ...
  string snudir =  getenv("LQANALYZER_DIR");
...
void AnalyzerCore::SetupLuminosityMap(bool initialsetup, TString forceperiod){
  ...
  lumitriggerpath=snudir + "/data/Luminosity/"+getenv("yeartag")+"/triggers_catversion_" + getenv("CATVERSION")+"_272007_275376.txt";
  /*
...
std::vector<snu::KJet> AnalyzerCore::GetJets(TString jetid, float ptcut, float etacut){
  ...
  // FIX
  eventbase->GetJetSel()->SelectJets(jetColl, GetMuons("MUON_POG_TIGHT"), GetElectrons("MVA_W_ISO") , ptcut,etacut);
...
std::vector<snu::KJet> AnalyzerCore::GetJetsWFT(TString jetid,TString fatjetid,float ptcut, float etacut){
  ...
  // Fix
  eventbase->GetJetSel()->SelectJets(jetColl, fatjetColl,GetMuons("MUON_POG_TIGHT"), GetElectrons("MVA_W_ISO") , ptcut,etacut);
...
std::vector<snu::KFatJet> AnalyzerCore::GetFatJets(TString fatjetid, float ptcut, float etacut){
  ...
  //Fix
  eventbase->GetFatJetSel()->SelectFatJets(fatjetColl,GetMuons("MUON_POG_TIGHT"), GetElectrons("MVA_W_ISO") , ptcut,etacut);
...
std::vector<snu::KMuon> AnalyzerCore::GetMuons(TString muid, bool keepfakes, float ptcut, float etacut){
  ...
  if(ptcut == -999.)   return GetMuons(muid, true, 10., 2.5);
  if(etacut == -999.)   return GetMuons(muid, true, ptcut, 2.5);

  std::vector<snu::KMuon> muonColl;

  // if muon  id is not set return empty vector  
  if(muid.Contains("NONE")) return muonColl;

  eventbase->GetMuonSel()->SelectMuons(muonColl ,muid, ptcut, etacut);
...
std::vector<snu::KElectron> AnalyzerCore::GetElectrons(bool keepcf, bool keepfake,  TString elid,float ptcut, float etacut){
  ...
  if(ptcut == -999.) eventbase->GetElectronSel()->SelectElectrons(electronColl,elid,10., 2.5);
  if(etacut == -999.) eventbase->GetElectronSel()->SelectElectrons(electronColl,elid,ptcut, 2.5);
  eventbase->GetElectronSel()->SelectElectrons(electronColl,elid, ptcut, etacut);
...
void AnalyzerCore::SetupDDBkg(){
  ...
  if(k_classname == "SKTreeMaker")  {
    m_logger << DEBUG << "MCDataCorrections setup" << LQLogger::endmsg;
    mcdata_correction = new MCDataCorrections();
    m_logger << DEBUG << "MCDataCorrections setup success " << LQLogger::endmsg;
  }
  ...
  if(!k_classname.Contains("SKTreeMaker")){
    ...
    m_logger << DEBUG << "SetupBTagger" << LQLogger::endmsg;

    //MapBTagSF = SetupBTagger(vtaggers,v_wps);
    m_logger << DEBUG << "SetupBTagger success" << LQLogger::endmsg;
    
    if(1){
      cout << getenv("yeartag") << endl;
      cout << getenv("CATVERSION") << endl;
    ...
    m_logger << DEBUG << "SetupLuminosityMap setup" << LQLogger::endmsg;

    SetupLuminosityMap(true);
    m_logger << DEBUG << "SetupLuminosityMap setup success" << LQLogger::endmsg;
...
void AnalyzerCore::SetUpEvent(Long64_t entry, float ev_weight) throw( LQError ) {
  ...
  m_logger << DEBUG << "SetupID" << LQLogger::endmsg;

  if(!IDSetup)   SetupID();

  m_logger << DEBUG << "SetupDDBkg" << LQLogger::endmsg;
...
bool AnalyzerCore::PassTrigger(TString trig){
  ...
  list.push_back(trig);
  int  prescaler=1.;
  return TriggerSelector(eventbase->GetTrigger().GetHLTInsideDatasetTriggerNames(),list);
...
bool AnalyzerCore::PassID(snu::KMuon mu, TString muid){
  ...
  return eventbase->GetMuonSel()->PassUserID(muid,mu);
...
bool AnalyzerCore::PassID(snu::KElectron el, TString elid){
  ...
  return eventbase->GetElectronSel()->PassUserID(elid,el);
...
void AnalyzerCore::SetCorrectedMomentum(vector<snu::KMuon>& k_muons){
  ...
  for(std::vector<snu::KMuon>::iterator it = k_muons.begin(); it != k_muons.end(); it++){
    ...
    if(it->RochPt() < 0.){
      /// If not loose muon then it can crash (this is safe unless using FLATLQ)
      if(it->IsPF() && (it->IsGlobal()==1 || it->IsTracker() == 1)&& it->Pt() > 5. && fabs(it->Eta()) < 2.5){
