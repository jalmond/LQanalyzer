...
class AnalyzerCore : public LQCycleBase {
 ...
 public:
    ...
    //	double TopMuIDSF( std::vector<snu::KMuon> mu, int syst);
    //  double TopMuIsoSF( std::vector<snu::KMuon> mu, int syst);
    //	double TopMuTriggerSF( std::vector<snu::KMuon> mu, int syst);
	...
	double ElectronScaleFactor( double eta, double pt, bool tight=true);
    ...
    map<TString, TNtuple*> mapntp;
    map<TString, TProfile*> mapprof;
  
    map<TString, TProfile2D*> mapprof2D; 	
	...
	void MakeHistograms3D(TString hname, int nbinsx, float xmin, float xmax, int nbinsy, float ymin, float ymax, int nbinsz, float zmin, float zmax);
  ...
  void MakeProfile2D(TString hname, int nbinsx, float xmin, float xmax, int nbinsy,float ymin, float ymax, float zmin, float zmax);
    //
    // Makes temporary dir
    //
    TDirectory* GetTemporaryDirectory(void) const;                                                                                                                                 
    ...
    TNtuple* GetNtp(TString hname);
    TProfile* GetProfile(TString hname);
    TProfile2D* GetProfile2D(TString hname);	
    ...
    void FillHist(TString histname, float value1,  float value2, float value3, float w , float xmin, float xmax, int nbinsx,  float ymin, float ymax, int nbinsy, float zmin, float zmax, int nbinsz );
    
    /// Fills ntuple
  void FillNtp(TString hname, Float_t myinput[]);

  void FillProfile(TString hname, float xvalue, float yvalue, float w);
  void FillProfile(TString hname, float xvalue, float yvalue, float zvalue, float w);  
	/// Fills clever hists
