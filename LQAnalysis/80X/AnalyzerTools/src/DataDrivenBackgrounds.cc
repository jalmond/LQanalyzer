...
bool DataDrivenBackgrounds::SetupFake(TString path, std::map<TString, std::pair<std::pair<TString,TString>  ,std::pair<float,TString> > >fake_hist_config){
 ...
 void DataDrivenBackgrounds::CheckEventBase(){
   ...
   dd_eventbase->GetMuonSel()->SelectMuons(muonColl,"MUON_POG_TIGHT",true);
...
float DataDrivenBackgrounds::Get_DataDrivenWeight_EM(bool geterr, vector<snu::KElectron> k_electrons, vector<snu::KMuon> k_muons, TString IDel, TString IDmu, TString variable,TString tagE, TString tagM, int FakeType){
   ...
   float val1(0.), val2(0.);
   if(variable=="ptcone"){
     val1 = k_electrons[0].PTCone(_electron_isocone,dd_eventbase->GetElectronSel()->IsoCutValue(k_electrons.at(0),IDel, "reliso_03"));
     val2 = k_muons[0].PTCone(_muon_isocone,dd_eventbase->GetMuonSel()->IsoCutValue(k_muons.at(0),IDmu,"reliso_03"));     
   }
   else if(variable=="pt"){
     val1=  electrons[0].Pt();
     val2=  muons[0].Pt();
...
float DataDrivenBackgrounds::Get_DataDrivenWeight_MM(bool geterr, vector<snu::KMuon> k_muons,  TString ID, TString variable, TString tag , int FakeType){
  ...
  if(k_muons.size()==2){
    ...
    if(variable=="ptcone"){
      val1 = k_muons[0].PTCone(_muon_isocone,dd_eventbase->GetMuonSel()->IsoCutValue(k_muons.at(0),ID,"reliso_03"));
      val2 = k_muons[1].PTCone(_muon_isocone,dd_eventbase->GetMuonSel()->IsoCutValue(k_muons.at(1),ID,"reliso_03"));
    }
    else if(variable=="pt"){
      val1=  muons[0].Pt();
      val2=  muons[0].Pt();
...
float DataDrivenBackgrounds::Get_DataDrivenWeight_M(bool geterr, vector<snu::KMuon> k_muons, TString IDmu, TString tag){
  ...
  float f=  m_fakeobj->getFakeRate(k_muons[0].PTCone(_muon_isocone,dd_eventbase->GetMuonSel()->IsoCutValue(k_muons.at(0),IDmu,"reliso_03")),fabs(k_muons.at(0).Eta()), tag,10.);
  float r=  1.;
...
float DataDrivenBackgrounds::Get_DataDrivenWeight_EE(bool geterr, vector<snu::KElectron> k_electrons,  TString ID, TString variable, TString tag , int FakeType){
  ...
  if(k_electrons.size()==2){
    ...
    if(variable=="ptcone"){
      val1 = k_electrons[0].PTCone(_electron_isocone,dd_eventbase->GetElectronSel()->IsoCutValue(k_electrons.at(0),ID,"reliso_03"));
      val2 = k_electrons[1].PTCone(_electron_isocone,dd_eventbase->GetElectronSel()->IsoCutValue(k_electrons.at(1),ID,"reliso_03"));
    }
    else if(variable=="pt"){
