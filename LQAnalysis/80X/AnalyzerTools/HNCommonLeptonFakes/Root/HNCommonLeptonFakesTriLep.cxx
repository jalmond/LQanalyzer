...
/// local includes
#include "HNCommonLeptonFakesTriLep/HNCommonLeptonFakesTriLep.h"

using namespace std;


void HNCommonLeptonFakesTriLep::InitialiseFake(){
  
  bool failedinitialisation=false;

  cout << "HNCommonLeptonFakesTriLep : Initializing" << endl;
  ...
  if(failedinitialisation){
    ...
    cout << "WRHNCommonLeptonFakesTriLep : Initialsation was successful " << endl;
      cout << "WRHNCommonLeptonFakesTriLep : Default setting in MeV" <<  endl;
      cout << "WRHNCommonLeptonFakesTriLep : To Use GeV set third argument of constructor true" << endl;
    }
...
/// Constructor
HNCommonLeptonFakesTriLep::HNCommonLeptonFakesTriLep(std::string path,bool usegev){
...
/// Destructor
HNCommonLeptonFakesTriLep::~HNCommonLeptonFakesTriLep() {
  closefile();   
  CutHists.clear();
}
...
float HNCommonLeptonFakesTriLep::get_dilepton_ee_eventweight(bool geterr, std::vector<TLorentzVector> electrons, bool isel1tight, bool isel2tight, vector<TString> cut){
...
float HNCommonLeptonFakesTriLep::get_dilepton_ee_eventweight(bool geterr, std::vector<TLorentzVector> electrons, bool isel1tight, bool isel2tight, TString cut1, TString cut2){
  if(electrons.size()!=2) {
  ...
  if(m_debug){
    cout << "HNCommonLeptonFakesTriLep::Event Summary (ee) " << endl;
...
float HNCommonLeptonFakesTriLep::get_dilepton_ee_eventweight(bool geterr, std::vector<TLorentzVector> electrons, bool isel1tight, bool isel2tight, TString cut){
...
float  HNCommonLeptonFakesTriLep::get_dilepton_em_eventweight(bool geterr, std::vector<TLorentzVector> muons, std::vector<TLorentzVector> electrons, bool isel1tight, bool ismu1tight ){
...
float HNCommonLeptonFakesTriLep::getEfficiency_electron(int sys, float pt, float eta){
...
float HNCommonLeptonFakesTriLep::getEfficiency_muon(int sys, float pt, float eta){
...
float HNCommonLeptonFakesTriLep::getPromptRate_electron(int sys,float pt, float eta, TString cut){
  float eff_prompt=1.;
  if(pt < 15.) pt = 16.;
  if(pt >= 200.) pt = 199.;
...
float HNCommonLeptonFakesTriLep::getFakeRate_electronEta(int sys,float pt, float eta, TString cut){
...
float HNCommonLeptonFakesTriLep::getFakeRate_electron(int sys,float pt, float eta ){
  float eff_fake=0.;
...
float HNCommonLeptonFakesTriLep::getFakeRate_muon(int sys,float pt, float eta , TString ID){
...
void HNCommonLeptonFakesTriLep::NoHist(TString hist, TString function){
...
void HNCommonLeptonFakesTriLep::NullTotals(){
  fsys=0;
  rsys=0;
...
void HNCommonLeptonFakesTriLep::CheckFile(TFile* file){

  if(file) cout << "HNCommonLeptonFakesTriLep: File " << file->GetName() << " was found." << endl;
  else cout << "HNCommonLeptonFakesTriLep  " << file->GetName()  << "  : ERROR Rootfile failed to open." << endl;
...
float HNCommonLeptonFakesTriLep::CalculateDiLepMMWeight(float r1, float fr1, float r2, float fr2, bool mu1Tight, bool mu2Tight){
...
float HNCommonLeptonFakesTriLep::CalculateDiLepMMWeight(float fr1_err, float fr1, float fr2_err, float fr2, bool mu1Tight, bool mu2Tight, int eventtype){
...
double HNCommonLeptonFakesTriLep::lepton_weight(bool loose, double r, double f) {
...
void HNCommonLeptonFakesTriLep::APPLY_scalefactor(bool applysf) {
  apply_scalefactor = applysf;
}

TString HNCommonLeptonFakesTriLep::DoubleToTString(double this_dXYSig, double this_RelIso){
...
void HNCommonLeptonFakesTriLep::SetTrilepWP(double this_dXYSig, double this_RelIso){
...
void HNCommonLeptonFakesTriLep::SetUseQCDFake(bool useit){
  UseQCDFake = useit;
}

void HNCommonLeptonFakesTriLep::SetDataPeriod(TString period){
  DataPeriod = period;
}

void HNCommonLeptonFakesTriLep::SetNJet(int nj){
  n_jet = nj;
}

void HNCommonLeptonFakesTriLep::SetNBJet(int nbj){
  n_bjet = nbj;
}


float HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon(bool geterr, float pt,  float eta){
  
  //cout << "[HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon] pt = " << pt << endl;
  //cout << "[HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon] eta = " << eta << endl;
  ...
  if(UseJetConf && UseBjetConf){
    cout << "[HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon] UseJetConf == true && UseBjetConf == true. Using alljet config.." << endl;
    wp = wp+"_alljet";
  }
    ...
    cout << "[HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon] ??" << endl;
  }
  ...
  if(UseQCDFake){
    ...
    if( mapit_FR==_2DEfficiencyMap_Double.end() ){
      ...
      //cout << "[HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon] FR = " << hist_FR->GetBinContent(binx) << endl;
      //cout << "[HNCommonLeptonFakesTriLep::getTrilepFakeRate_muon] FRerr = " << hist_FR->GetBinError(binx) << endl;
...
float HNCommonLeptonFakesTriLep::getTrilepPromptRate_muon(bool geterr, float pt, float eta){
  ...
  if(DataPeriod=="B"||DataPeriod=="C"||DataPeriod=="D"||DataPeriod=="E"||DataPeriod=="F"){
    ...
    cout << "[HNCommonLeptonFakesTriLep::getTrilepPromptRate_muon] DataPeriod is not correct" << endl;
    cout << "[HNCommonLeptonFakesTriLep::getTrilepPromptRate_muon] Using BCDEF PromptRate" << endl;
    mapit = _2DEfficiencyMap.find("MUON_PR_HN_TRI_TIGHT_BCDEF");
  }
...
std::vector<double> HNCommonLeptonFakesTriLep::GetdXYMins(){
  return dXYMins;
}

std::vector<double> HNCommonLeptonFakesTriLep::GetRelIsoMaxs(){
  return RelIsoMaxs;
}

int HNCommonLeptonFakesTriLep::GetNLooseNotTight(){

  return n_Loose_not_Tight;

}

void HNCommonLeptonFakesTriLep::SetUsePtCone(bool b){
...
float HNCommonLeptonFakesTriLep::get_trilepton_mmm_eventweight(bool geterr, std::vector<TLorentzVector> muons, bool isT1, bool isT2, bool isT3){
...
float HNCommonLeptonFakesTriLep::get_dilepton_mm_eventweight(bool geterr, std::vector<TLorentzVector> muons, bool isT1, bool isT2){
  return get_dilepton_mm_eventweight("dxy", geterr,  muons, isT1,isT2,"");
}

float HNCommonLeptonFakesTriLep::get_dilepton_mm_eventweight(TString fakerates, bool geterr, std::vector<TLorentzVector> muons, bool isT1, bool isT2, TString ID1,TString ID2, bool c1, bool c2, TString method, float ptcorr1, float ptcorr2,bool checkcj){
...
TDirectory* HNCommonLeptonFakesTriLep::getTemporaryDirectory(void) const
{
  //                                                                                                                                                                                
  ...
  while (not tempDir) {
    ...
    dirname << "HNCommonLeptonFakesTriLep_%i" << counter;
...
float HNCommonLeptonFakesTriLep::get_eventweight(bool geterr, std::vector<TLorentzVector> muons, TString muid, std::vector<TLorentzVector> electrons, vector<TString> elcut, std::vector<bool> isT, int HalfSampleErrorDir){
