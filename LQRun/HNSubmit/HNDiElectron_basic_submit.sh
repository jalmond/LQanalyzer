...
then
    ...
    cycle="HNDiElectron_basic_basic"
    skinput="True"
    useskim="DiLep"
    njobs=1
    data_lumi="AtoD"
...
if [[ $runfakes  == "true" ]];
    ...
    ### submit this configured job (uses bin/submit.sh)
    source submit.sh $1
    source hadd.sh ${LQANALYZER_DIR}/data/output/SSElectron/  HNDiElectron_basic_SKnonprompt_dilep_5_3_14.root HNDiElectron_basic_nonprompt_period*
fi
