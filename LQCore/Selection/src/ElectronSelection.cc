...
void ElectronSelection::TopVetoElectronSelectionv1(std::vector<KElectron>& leptonColl, bool m_debug)
{
  std::vector<KElectron> allelectrons = k_lqevent.GetElectrons();
  double rho = k_lqevent.GetEvent().JetRho();
  ...
  for (std::vector<KElectron>::iterator el = allelectrons.begin(); el!=allelectrons.end(); el++){
    ...
    if(!(LeptonRelIsoDR03 <  0.3)){                                                          
         
      pass_selection = false;                                                             
      if(m_debug)  cout << "HNVetoElectronSelection:Fail Isolation Cut" <<endl;           
...
void ElectronSelection::TopTightElectronSelectionv1(std::vector<KElectron>& leptonColl, bool m_debug   ) {
  ...
  for (std::vector<KElectron>::iterator el = allelectrons.begin(); el!=allelectrons.end(); el++){
    ...
    if(!(LeptonRelIsoDR03 <  0.3)){ //use 0.1 in analysis code 
      pass_selection = false;
...
void ElectronSelection::TopVetoElectronSelection(std::vector<KElectron>& leptonColl, bool m_debug) {
  ...
  for (std::vector<KElectron>::iterator el = allelectrons.begin(); el!=allelectrons.end(); el++){
    ...
    float LeptonRelIsoDR03(0.);                                                                        
    float ElectronIsoDR03 =  el->PFChargedHadronIso03() + max( el->PFNeutralHadronIso03() + el->PFPhot\
onIso03() - rho * PHONH_03[ifid],  0.);                                                                
    if(el->Pt() > 0.)  LeptonRelIsoDR03 = ElectronIsoDR03/  el->Pt();
