...
Double_t TTFitter::Mass[24]; //24 combinations. only one Mtop from the fitter for each event
Double_t TTFitter::HMass[24]; //24 combinations. only one Mtop from the fitter for each event
Double_t TTFitter::Chi2[24];
Double_t TTFitter::Error[24][9]; //one error for each parameter for each combination
Double_t TTFitter::mtop_error_p[24];
Double_t TTFitter::mtop_error_m[24];
Double_t TTFitter::bestchi2;
Double_t TTFitter::bestmass;
Double_t TTFitter::bestHmass;
Double_t TTFitter::neupzin[24]; //neutrino pz solution used by the fitter for each of the 24 combinations
Int_t TTFitter::sol;
Int_t TTFitter::combos[24][4];
bool TTFitter::tagconsistent[24];
TLorentzVector TTFitter::rawJets[24][4];
TLorentzVector TTFitter::jetsIN[24][4];

bool TTFitter::imaginary[24];

TLorentzVector TTFitter::FittedJets[24][4];
TLorentzVector TTFitter::FittedLep[24];
TLorentzVector TTFitter::FittedNeu[24];
Double_t TTFitter::JetSigma[24][4];
...
Double_t TTFitter::LeptonChi[24];
Double_t TTFitter::UnclXChi[24];
Double_t TTFitter::UnclYChi[24];
Double_t TTFitter::LepWChi[24];
Double_t TTFitter::HadWChi[24];
Double_t TTFitter::HadTopChi[24];
Double_t TTFitter::LepTopChi[24];
Double_t TTFitter::JetChi[4][24];

Double_t TTFitter::Scale[24][7];
Double_t TTFitter::Out[24][10];
...
void TTFitter::Fit(TLorentzVector Lepton, float Lepsigma, TLorentzVector jets[], TLorentzVector jetraw[], bool btag[], TLorentzVector sumjets, TLorentzVector met, int ntupletype,  Int_t tstype, bool debug){
	...
	Double_t addljetsEx=sumjets.Px(),addljetsEy=sumjets.Py(),addljetsEt=sumjets.Pt();
	...
	addljets.SetPxPyPzE(addljetsEx,addljetsEy,0,addljetsEt);
	...
	unclrawEx -= addljets.Px();
	unclrawEy -= addljets.Py();
	...
	Double_t vstart[nfit]={1.0,1.0,1.0,1.0,Topmass,1.0,1.0,1.0,0.0,Wmass}; //modified version

	//step size of the parameters
	...
	Double_t step[nfit]  ={0.03, 0.03, 0.03, 0.03, 0.01, 0.01, 0.1, 0.1, 1.0, 0.01};//modified version
	...
	TString name[nfit]={"b1","b2","j1","j2","TopMass","lep","unclx","uncly","Pzneu","HMass"};
	...
	for(int i=0;i<24;i++){
		...
		if(i>0){
			oddnumber=!oddnumber;
			std::next_permutation(combo,combo+4);
		}
		...
		Double_t low[nfit]={1.0-2.0*ratio[0],1.0-2.0*ratio[1],1.0-2.0*ratio[2],1.0-2.0*ratio[3],0.,0.99,0.5,0.5,0.,0.};
		Double_t high[nfit]={1.0+2.0*ratio[0],1.0+2.0*ratio[1],1.0+2.0*ratio[2],1.0+2.0*ratio[3],0.,1.01,1.5,1.5,0.,0.};
		...
		if(PzSol[0]==PzSol[1] && oddnumber) continue;
		...
		if(btag[combo[0]]) b0=true;
		if(btag[combo[1]]) b1=true;
		if(btag[combo[2]]) b2=true;
		...
		for(int j=0;j<4;j++){
			...
			if(debug) {
				std::cout << "top specific jet corrections " << jets[combo[j]].Pt() << " "  
		...
		minuit->mnexcm("MINIMIZE",arglist,1,ierflg); // calls simplex, then migrad if this fails
		...
		//    if(!constrained) minuit->mnexcm("MINOS",arglist,2,ierflg); // -- if unconstrained, calculates the MINOS errors on parameter [4]
...
void TTFitter::fcn(Int_t &npar,Double_t *gin,Double_t &f, Double_t *par,Int_t iflag){
	...
	Double_t neuPx = -SumJetPx - fitlep.Px() - par[6]*uncl.Px();
	Double_t neuPy = -SumJetPy - fitlep.Py() - par[7]*uncl.Py();
