...
bool KFatJet::PassCustomID(TString IDlabel){
  ...
  // -- basic pt/eta cuts only
  
  if(CheckIDList(IDlabel , "JET_PTETA",IDlist)){
    if(this->Pt() < 20)pass=false;
  ...
  else if(CheckIDList(IDlabel , "HN_NN_JET",IDlist)){
    if(!this->PassCustomID("JET_PTETA")) pass=false;
    if(!this->PassTightID())pass=false;
    if(this->SoftDropMass() < 60.) pass=false;
    return pass;
  }
  if(IDlabel == "") return true;
  else   {
    std::cout  << "KFatJet::PassCustomID(ID) " << IDlabel << "  wrong IDlabel."   <<  std::endl;
    std::cout  << "-------------------------------------------------------------------" <<  std::endl;
