...
std::vector<KElectron> SKTreeFiller::GetAllElectrons(){
  ...
  for (UInt_t iel=0; iel< electrons_eta->size(); iel++) {
    ...
    if(electrons_isGsfScPixChargeConsistent)el.SetGsfScPixCharge(electrons_isGsfScPixChargeConsistent->at(iel));
    if(electrons_isGsfCtfChargeConsistent)el.SetGsfCtfCharge(electrons_isGsfCtfChargeConsistent->at(iel));
	...
	unsigned int mindex= matched_index;
	...
	if( (fabs(gen_pdgid->at(mindex)) == 23) || (fabs(gen_pdgid->at(mindex)) == 24)) {
	  ...
	  if(gen_status->at(mindex) == 2){
		...
		unsigned int index_mother_nu=gen_motherindex->at(itx);
