...
SignalPlotsEM::SignalPlotsEM(TString name): StdPlots(name){
  ...
  map_sig["h_oseemass"]               =     new TH1D("h_osllmass_"          + name,"Invariant mass of the two leading os leptons",100,0,500);
  ...
  map_sig["h_leadingLeptonIso"]    =     new TH1D("h_leadingLeptonRelIso_"      + name,"leading lepton relIso",100,0,1.);
  map_sig["h_secondLeptonPt"]         =     new TH1D("h_secondLeptonPt_"    + name,"secondary lepton pt",60,0,300);
  map_sig["h_secondLeptonIso"]    =     new TH1D("h_secondLeptonRelIso_"      + name,"second lepton relIso",100,0,1.);
...
void SignalPlotsEM::Fill(snu::KEvent ev, std::vector<snu::KMuon>& muons, std::vector<snu::KElectron>& electrons, std::vector<snu::KJet>& jets, Double_t weight) {
  ...
  if(nlep!=0)Fill("h_leadLeptondR",min_ee_Dr,weight);
