...
MuonPlots::MuonPlots(TString name) : StdPlots(name) {
  ...
  map_muon["h_charge"] = new TH1D ("h_charge" + name , "",6, -3  , 3. );
  map_muon["h_ispf"] = new TH1D ("h_ispf" + name ,"",2,0.  , 2.);
  map_muon["h_isglobal"] = new TH1D ("h_isglobal" + name ,"",2,0.  ,2. );
  map_muon["h_istracker"] = new TH1D ("h_istracker" + name ,"", 2,0.,2. );
  map_muon["h_validhits"] = new TH1D ("h_validhits" + name ,"", 40,0.  ,40. );
  map_muon["h_validpixhits"] = new TH1D ("h_validpixhits" + name ,"",10,0.  ,10.);
  map_muon["h_validstations"] = new TH1D ("h_validstations" + name ,"",10,0. ,10. );
  map_muon["h_activelayers"] = new TH1D ("h_activelayers" + name ,"", 20,0.  , 20.);
  map_muon["h_muonvtx"] = new TH1D ("h_muonvtx" + name ,"",1000,-3.  ,3. );
  ...
  map_muon["h_istight"] = new TH1D ("h_istight" + name ,"",2,0.  ,2. );
  map_muon["h_issoft"] = new TH1D ("h_issoft" + name ,"",2,0.  ,2. );
  map_muon["h_mcmatched"] = new TH1D ("h_mcmatched" + name ,"",2,0.  ,2. );
  map_muon["h_mciscf"] = new TH1D ("h_mciscf" + name ,"",2,0.  ,2. );
  map_muon["h_mcisconv"] = new TH1D ("h_mcisconv" + name ,"",2,0.  ,2. );
  map_muon["h_mcfromtau"] = new TH1D ("h_mcfromtau" + name ,"",2,0.  , 2.  );
  map_muon["h_mcisprompt"] = new TH1D ("h_mcisprompt" + name ,"",2,0.  ,2. );
  map_muon["h_mcmatched_pdgid"] = new TH1D ("h_mcmatched_pdgid" + name ,"",200,-100.  ,100. );
  map_muon["h_mother_pdgid"] = new TH1D ("h_mother_pdgid" + name ,"", 200,-100.  ,100. );
  map_muon["h_mother_index"] = new TH1D ("h_mother_index" + name ,"",100,0.  ,100. );
  map_muon["h_mc_index"] = new TH1D ("h_mc_index" + name ,"",100,0.  ,100. );
  map_muon["h_reliso03"] = new TH1D ("h_reliso3" + name ,"",100,0.  ,1. );
  map_muon["h_reliso04"] = new TH1D ("h_reliso4" + name ,"",100,0.  ,1.);
  map_muon["h_ptshifted_up"] = new TH1D ("h_ptshifted_up" + name ,"",250,0.  ,500. );
  map_muon["h_ptshifted_down"] = new TH1D ("h_ptshifted_down" + name ,"",250,0.  , 500.  );
  map_muon["h_particle_type"] = new TH1D ("h_particle_type" + name ,"",10,0.  ,10. );
  map_muon["h_mother_type"] = new TH1D ("h_mother_type" + name ,"",10,0.  ,10  );
  map_muon["h_pt"] = new TH1D ("h_pt" + name ,"",250,0.  ,500. );
  map_muon["h_eta"] = new TH1D ("h_eta" + name ,"",60,-3.  ,3. );
  map_muon["h_phi"] = new TH1D ("h_phi" + name ,"",70,-3.5  ,3.5  );
  map_muon["h_energy"] = new TH1D ("h_energy" + name ,"",250,0.  ,500.  );
  map_muon["h_mass"] = new TH1D ("h_mass" + name ,"",250,0.  ,500.0 );
