...
void GetEffectiveLuminosity(TString version="") {
  ...
  ofstream lumi_file2;
  string lfile3 =  "dataset_list_missing_mcsamples_" + string(version.Data()) + ".txt";
  lumi_file2.open(lfile.c_str());
  lumi_file2.setf(ios::fixed,ios::floatfield);
  lumi_file2.precision(1);
  
  lumi_file2 << "######################################################" << endl;
  lumi_file2 << "#### Missing samples in this version are listed below " << endl;
  lumi_file2 << "######################################################" << endl;
  for(unsigned int i=0; i < missing_samples.size() ; i++){
    if(!NotProduced(missing_samples.at(i))) lumi_file2 << missing_samples.at(i) << endl;
  }
  lumi_file2 << "" << endl;
  lumi_file2 << "######################################################" << endl;
  lumi_file2 << "#### Samples available samples in this version are listed below " << endl;
  lumi_file2 << "######################################################" << endl;
...
bool NotProduced(TString name){
  ...
  if(name.Contains("GluGluToZZ")) return true;
  if(name.Contains("mtop1"))  return true;
  if(name.Contains("TuneEE5C"))  return true;
  if(name.Contains("scale"))  return true;
  return false;
