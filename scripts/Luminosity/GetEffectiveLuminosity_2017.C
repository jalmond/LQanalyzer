...
void GetEffectiveLuminosity_2017(TString path_of_list,  TString tag,TString version="") {
  ...
  bool NewList(true);
  if (path_of_list.Contains(string(getenv("LQANALYZER_DATASET2017_DIR")) +"/ca")) NewList=false;

  if (NewList) cout << "New list " << endl;
  else cout <<"Not new list" << endl;

  if(CheckMaps(path_of_list)) return;
  TString def_version = "v9-4-9v2";//TString(getenv("CATVERSION"));
  if(!version.Contains("v9") ) version = def_version;
