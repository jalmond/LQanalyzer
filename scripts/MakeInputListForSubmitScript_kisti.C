...
void MakeInputListForSubmitScript_kisti(){
  ...
  for(std::map<TString, TString>::iterator mit =lqmap_tmp.begin(); mit != lqmap_tmp.end();++mit){
    ...
    TString dir = "ls /xrootd/store/user/jalmond/cattoflat/MC/" + def_version + "/"+ mit->first + "/*.root > inputlist_lists.txt";
  ...
  for(std::map<TString, TString>::iterator mit =lqmap.begin(); mit != lqmap.end();++mit){
    ...
    if(std::next(mit, 1) != lqmap.end()) lumi_file << mit->second << "' '"   ;
 ...
 for(std::map<TString, TString>::iterator mit =lqmap.begin(); mit != lqmap.end();++mit){
  ...
  for(std::map<TString, TString>::iterator mit =lqmap.begin(); mit != lqmap.end();++mit){
    ...
    if(mit->second.Contains("mup")) continue;
    bool last=true;
    for(std::map<TString, TString>::iterator mit2 =std::next(mit, 1); mit2 != lqmap.end();++mit2){
