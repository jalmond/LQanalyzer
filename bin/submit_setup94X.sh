...
function rungroupedlist
{
    ...
    done < ${LQANALYZER_DIR}/LQRun/txt/list_all_mc_${SKFLATVERSION}.sh
...
function runlist
{
	...
	for ic in  ${list_of_catversions[@]};
	  ...
	  echo "Following samples are available in Catversion: " ${ic} " but not in: " $SKFLATVERSION
...
#                                ;;
	...
	-h | --help )        	shift
				...
				if [[ $other_commands == "" ]];
				    ...
				    echo "       |                                                      |                     |                                     |"
				    echo "-c     |   catversion of inputfile                            | default = ${SKFLATVERSION}    | (only needed if not running         |"
				    echo "       |                                                      |                     | default/latest)                     | " 
                                    echo "-ac    |   true/false                                         | default = 'false'   | Check all catversions for input     | "
                                    echo "       |                                                      |                     | Rare: only set true if a sample is  |" 
                                    echo "       |                                                      |                     | not available in  ${SKFLATVERSION}            |"
                                    echo "       |                                                      |                     |                                     |" 
				...
				if [[ $other_commands == "more" ]];
				    ...
				    echo "-D       | any allowed  SKFLATVERSION                            | default = $SKFLATVERSION    | returns Info on Catuple production.|"
				    ...
				    echo "-A       | can speficy catversion and search                  | default = ${SKFLATVERSION}    | lists missing samples due to no     |"
...
if [[ $job_data_lumi == "ALL" ]];
    ...
    if [[ $SKFLATVERSION == "v8-0-8" ]];then
        declare -a data_periods=("B" "C" "D" "E" "F" "G" "H_v2" "H_v3")
    fi
    if [[ $SKFLATVERSION == "v8-0-7" ]];then
        declare -a data_periods=("B" "C" "D" "E" "F" "G" "H_v2" "H_v3")
    fi
    if [[ $SKFLATVERSION == "v8-0-6" ]];then
        declare -a data_periods=("B" "C" "D" "E" "F" "G" "H_v2" "H_v3")
    fi

    if [[ $SKFLATVERSION == "v8-0-4" ]];then
        declare -a data_periods=("B" "C" "D" "E" "F" "G" "H_v2" "H_v3")
    fi
    if [[ $SKFLATVERSION == "v8-0-3" ]];then
	declare -a data_periods=("B" "C" "D" "E" "F" "G" "H_v2" "H_v3")
    fi
    if [[ $SKFLATVERSION == "v8-0-2" ]];then
        declare -a data_periods=("B" "C" "D" "E" "F" "G")
    fi

    if [[ $SKFLATVERSION == "v8-0-1" ]];then
        declare -a data_periods=("B" "C" "D" "E")
    fi
    if [[ $SKFLATVERSION == "v7-6-6" ]];then
...
if [[ $MakeFullLists == "true" ]];
    ...
    echo $submit_version_tag " " ${TXTPATH}"CAT_mc_"${iclist}".txt"
    ##### LOOP OVER ALL SKFLATVERSIONS (ONLY STORE SAMPLES THAT EXIST AND NOT IN NEWER VERSIONS)
    for iclist in  ${list_of_catversions[@]};
      do
      #### IF CHECK ALL SKFLATVERSION  = FALSE WE LOOK AT ONLY $SKFLATVERSION
	echo $$check_all_catversions " " $iclist
      if [[ $check_all_catversions != "true" ]];
	  then
