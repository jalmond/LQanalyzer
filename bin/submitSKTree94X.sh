...
if [[ $HOSTNAME == "ui"* ]];
then
    source    /cms/scratch/SNU/CATAnalyzer/LQAnalyzer_rootfiles_for_analysis/CattupleConfig/${SKFLATVERSION}.sh
else
    source    /data1/LQAnalyzer_rootfiles_for_analysis/CattupleConfig/${SKFLATVERSION}.sh
fi

cp $LQANALYZER_DATASETFILE2017_DIR/datasets_snu* $LQANALYZER_DIR/LQRun/txt/
cp $LQANALYZER_DATASETFILE2017_DIR/list_all_mc*  $LQANALYZER_DIR/LQRun/txt/
...
submit_file_tag=""
submit_file_list=""
submit_version_tag=${SKFLATVERSION}
submit_sampletag=""
submit_catvlist=""
submit_searchlist=""
...
FLATCAT_MC="SKFlat/v949cand2_2/2017/MC/"
...
linkdef_filepath=/LQAnalysis/94X/Analyzers/include/LQAnalysis94X_LinkDef.h

if [[ $make_sktrees == "True" ]];
    then
    linkdef_filepath=/LQAnalysis/94X/SKTreeMaker/include/SKTreeMaker94X_LinkDef.h
fi

if [[ $run_validation == "True" ]];
    then
    linkdef_filepath=/LQAnalysis/94X/Validation/include/Validation94X_LinkDef.h
fi
...
if [[ $submit_file_tag  != ""  ]];
	...
	echo "LQanalyzer::sktree :: ERROR :: Samplename is Invalid for the catversion "$SKFLATVERSION" and skim "$job_skim":" 
	if [[ $check_all_catversions == "false" ]];
		...
		echo "LQanalyzer::sktree :: HELP :: Check new input options using 'sktree -L "$job_skim" "$SKFLATVERSION"'"
		exit 1
	    else
		echo ""
		echo "LQanalyzer::sktree :: HELP :: 1) Check input options using 'sktree -L "$job_skim" "$SKFLATVERSION"'"
...
outputdir_cat=$outdir"/data/output/SKFlat/"

outputdir_analyzer=$outdir"/data/output/SKFlat/"$submit_analyzer_name
...
then
	...
	echo "Copying "
	cp -r $LQANALYZER_DIR/LQAnalysis/94X/ $outputdir_mc
    fi
fi
