...
void MakeFRRootFile(){
  ...
  TFile * fdata = new TFile(path + "FakeRateCalculator_FinalEl_data_DoubleEG_cat_v8-0-7.root");
  TFile * fdataS = new TFile(path + "FakeRateCalculator_FinalEl_data_SingleElectron_cat_v8-0-7.root");
  TFile * fmc = new TFile(path + "FakeRateCalculator_FinalEl_mc_v8-0-7.root");
  ...
  TString outfile = "FakeRate13TeV_2016_hnid.root";
  TFile* fout = new TFile(outfile.Data(),"RECREATE");
  fout->cd();

  std::vector<TString> fakes40;
  fakes40.push_back("pt_eta");
  fakes40.push_back("ptcorr_eta");
  fakes40.push_back("0bjet_pt_eta");
  fakes40.push_back("bjet_pt_eta");
  //
  ...
  for(vector<TString>::iterator it2 = fakes40.begin(); it2!=fakes40.end(); ++it2){
    for(vector<TString>::iterator it3 = isocut.begin(); it3!=isocut.end(); ++it3){
      ...
      TString denom ="LooseElHN_TIGHT_"  + *it3 +"_"+ *it2;
      TString num ="TightElHN_TIGHT_"  +  *it3 + "_"+*it2;
  ...
  for(vector<TString>::iterator it2 = fakes40.begin(); it2!=fakes40.end(); ++it2){
    for(vector<TString>::iterator it3 = isocut.begin(); it3!=isocut.end(); ++it3){
      ...
      if(!CheckFile(fdataS))return;
      if(!CheckFile(fmc))return;
      TString denom ="LooseElsingleel_HN_TIGHT_"  + *it3 +"_"+ *it2;
      TString num ="TightElsingleel_HN_TIGHT_"  +  *it3 + "_"+*it2;

      TH2D* h_pt_num= (TH2D*)fdataS->Get(num.Data());
      TH2D* h_pt_denom= (TH2D*)fdataS->Get(denom.Data());
      ...
      TH2D* eff_rate = (TH2D*)h_pt_num->Clone(("FakeRate_SingleElTrigger_" + name).Data());
      TH2D* hratedenom = (TH2D*)h_pt_denom->Clone((name +"_denom").Data());
      eff_rate->Add(h_mcpt_num,-1.);
      hratedenom->Add(h_mcpt_denom, -1.);
      eff_rate->Divide(eff_rate,hratedenom,1.,1.,"cl=0.683 b(1,1) mode");
