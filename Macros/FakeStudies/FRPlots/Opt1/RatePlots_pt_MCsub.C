...
//#include "Macro.h"
void setTDRStyle();
TH1D* MakeFRPlot(TString sample, TString hist, TString cut);
...
void FR_pt(){
  ...
  setTDRStyle();
  TString cut="pt";
  TH1D* h = MakeFRPlot("","ElELECTRON16_FR_POG_MEDIUM_CC_dijet_pog_40_",cut);
  TH1D* h2 = MakeFRPlot("","ElELECTRON16_FR_POG_TIGHT_CC_dijet_pog_40_",cut);
  TH1D* h3 = MakeFRPlot("","ElELECTRON16_FR_MVA_TIGHT_CC_dijet_mva_40_",cut);
  ...
  c1->SaveAs("TL_mcsub_pt_pog_id.pdf");
}
...
TH1D* MakeFRPlot(TString sample, TString hist,TString cut){
  ...
  TH1D* hloose = (TH1D*)fdata->Get(("Loose"+hist+cut).Data());
  TH1D* htight = (TH1D*)fdata->Get("Tight"+hist+cut);


  TH1D* hmcloose = dynamic_cast<TH1D*>(fmc->Get("Loose"+hist+cut)->Clone("Loose"+hist+cut));
  TH1D* hmctight = dynamic_cast<TH1D*>(fmc->Get("Tight"+hist+cut)->Clone("Tight"+hist+cut));

  cout << "Loose"+hist+cut << hmcloose << endl;
  cout << "Tight"+hist+cut << hmctight << endl;
  
  if(!hloose) exit(1);
  if(!htight) exit(1);

  cout << hloose << " " << hmctight << endl;
  cout << hmcloose << " " << hmctight << endl;
  
  hloose->Add(hmcloose,-1);
  htight->Add(hmctight,-1);
