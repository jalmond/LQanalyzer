...
void FR_pt(){
  ...
  for(unsigned int i=0; i < detector_reg.size(); i++){
    ...
    TH1D* h_top = MakeFRPlot("GENT",detector_reg[i],"ptcorr1D_full",true);
    
    h->SetLineColor(kRed-2);
    h->GetXaxis()->SetRangeUser(5.,90.);
    h_top->SetLineColor(kBlue-2);
    ...
    legend->AddEntry(h_top,"GENT "+detector_reg2[i]+"(top)","l");
    ...
    c1->SaveAs("TL_mc_gentid"+detector_reg[i]+"_top_qcdpt.pdf");
  }
}
