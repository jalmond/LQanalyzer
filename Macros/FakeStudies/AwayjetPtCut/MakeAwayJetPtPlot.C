...
void MakeAwayJetPtPlot(TString path) {
  ...
  for(vector<TString>::iterator it = plotname.begin(); it!=plotname.end(); ++it,ihist++){
    ...
    for(vector<TString>::iterator it2 = hist_ptcut.begin(); it2!=hist_ptcut.end(); ++it2){
      ...
      cout << h_pt_num << " " << h_pt_denom << " " << h_mcpt_num << " " << h_mcpt_denom << endl;
    ...
    c1->SaveAs(("/home/jalmond/WebPlots/13TeV/Fakes/FakeRateEWKSub_jetcuts_" +  *it + ".pdf"));
  }
