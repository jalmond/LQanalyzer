...
void GetPDFUncert_600(){
  
  
  setTDRStyle();
  //gStyle->SetPalette(1);
  
  TString spath = "/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee600_nocut_5_3_14.root";
  
  TFile * _file = new TFile(spath);
  
  TH1* h_cteq_total = (TH1*)_file->Get("limithist/500_defaultMassRegion_limithist_cteq");
  TH1* h_cteq_sum =  (TH1*)_file->Get("sum_cteq");
  h_cteq_total->Divide(h_cteq_sum);
  
  TH1* h_mstw_total = (TH1*)_file->Get("limithist/500_defaultMassRegion_limithist_mstw");
  TH1* h_mstw_sum =  (TH1*)_file->Get("sum_mstw");
  h_mstw_total->Divide(h_mstw_sum);
  
  TH1* h_nnpdf_total = (TH1*)_file->Get("limithist/500_defaultMassRegion_limithist_nnpdf");
  TH1* h_nnpdf_sum =  (TH1*)_file->Get("sum_nnpdf");
  h_nnpdf_total->Divide(h_nnpdf_sum);

  TH1* h_nominal = (TH1*)_file->Get("limithist/500_defaultMassRegion_limithist");
  ...
  c1->SaveAs(("/home/jalmond/WebPlots/PreApproval/PDFSyst/mN_600_acc_eff_pdf.pdf"));
