...
void GetPDFUncert_400(){
  
  
  setTDRStyle();
  //gStyle->SetPalette(1);
  
  TString spath = "/home/jalmond/HeavyNeutrino/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee400_nocut_5_3_14.root";
  
  TFile * _file = new TFile(spath);
  
  TH1* h_cteq_total = (TH1*)_file->Get("limithist/400_defaultMassRegion_limithist_cteq");
  TH1* h_cteq_sum =  (TH1*)_file->Get("sum_cteq");
  h_cteq_total->Divide(h_cteq_sum);
  
  TH1* h_mstw_total = (TH1*)_file->Get("limithist/400_defaultMassRegion_limithist_mstw");
  TH1* h_mstw_sum =  (TH1*)_file->Get("sum_mstw");
  h_mstw_total->Divide(h_mstw_sum);
  
  TH1* h_nnpdf_total = (TH1*)_file->Get("limithist/400_defaultMassRegion_limithist_nnpdf");
  TH1* h_nnpdf_sum =  (TH1*)_file->Get("sum_nnpdf");
  h_nnpdf_total->Divide(h_nnpdf_sum);

  TH1* h_nominal = (TH1*)_file->Get("limithist/400_defaultMassRegion_limithist");

  TH1F* h_ref= (TH1F*)_file->Get(("NoCut_sigeff"));
  float nom = h_nominal->GetBinContent(2)/h_ref->Integral();
  
  
  h_nnpdf_total->SetMarkerColor(kCyan);
  h_nnpdf_total->SetMarkerStyle(24);
  h_nnpdf_total->GetYaxis()->SetRangeUser(0.05, 0.3);
  h_nnpdf_total->GetYaxis()->SetTitle("Acc. #times Eff.");
  h_nnpdf_total->GetXaxis()->SetTitle("Errorsets");
  ...
  err_nnpdf = sqrt(err_nnpdf/105.)/2.;

  cout << "err_nnpdf = " << err_nnpdf << " " <<  nom<< " " << (err_nnpdf*100/nom) << endl;
  ...
  h_nnpdf_band->SetFillColor(kCyan);
  ...
  TLine *line = new TLine(h_cteq_total->GetBinLowEdge(h_cteq_total->GetXaxis()->GetFirst()),nom,h_cteq_total->GetBinLowEdge(h_cteq_total->GetXaxis()->GetLast()+1),nom);
  ...
  TLegend* legendH = new TLegend(0.6,0.6,0.8,0.8);
  ...
  c1->SaveAs(("/home/jalmond/WebPlots/PreApproval/PDFSyst/mN_400_acc_eff_pdf.pdf"));
