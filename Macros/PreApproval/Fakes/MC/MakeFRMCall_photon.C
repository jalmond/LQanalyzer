...
void MakeFRMCall_photon(){
  ...
  list_of_regions.push_back("Dilep_photon");
  ...
  for(unsigned int j = 0; j < list_of_regions.size(); j++){
    for(unsigned int i = 0; i < list_of_names.size(); i++){
      ...
      TH1F* h_qcd_loose= (TH1F*)fqcd->Get(("Singlelep_photon_MCLooseEl_"+list_of_names.at(i)).Data());
      TH1F* h_qcd_tight= (TH1F*)fqcd->Get(("Singlelep_photon_MCTightEl_"+list_of_names.at(i)).Data());
	...
	h_qcd_loose->Rebin(5.);
	if(usetop){
	  h_ttbar_tight->Rebin(5.);
	  h_ttbar_loose->Rebin(5.);
	}
	if(usew){
	  h_wjet_tight->Rebin(5.);
	  h_wjet_loose->Rebin(5.);
	}
      }
	...
	h_qcd_tight->GetXaxis()->SetBinLabel(2,"bjet:  #DeltaR< 0.4");
      ...
      c1->SaveAs(("/home/jalmond/WebPlots/PreApproval/Fakes/MCFake_all_photon_" + list_of_regions.at(j ) + "_" + list_of_names.at(i) + ".pdf").Data());
    }
