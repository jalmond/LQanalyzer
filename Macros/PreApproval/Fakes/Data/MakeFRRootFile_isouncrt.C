...
void MakeFRRootFile_isouncrt(){
  ...
  TString outfile = "FakeRate2015_isoerror.root";
  ...
  type.push_back("HNTight_iso05_");
  type.push_back("HNTight_iso07_");
  ...
  for(vector<TString>::iterator it = type.begin(); it!=type.end(); ++it){
    for(vector<TString>::iterator it2 = fakes40.begin(); it2!=fakes40.end(); ++it2){
      ...
      /// Get Nominal
      TString nom_denom ="LooseElHNTight_" +  *it2;
      TString nom_num ="TightElHNTight_" + *it2;
      
      TH2F* h_nom_pt_num= (TH2F*)fdata->Get(nom_num.Data());
      TH2F* h_nom_pt_denom= (TH2F*)fdata->Get(nom_denom.Data());

      TH2F* h_nom_mcpt_num= (TH2F*)fmc->Get(nom_num.Data());
      TH2F* h_nom_mcpt_denom= (TH2F*)fmc->Get(nom_denom.Data());

      TString nom_tag = "HNTight_" + *it2;
      TH2F* nom_eff_rate = (TH2F*)h_nom_pt_num->Clone(("FakeRate_nom_" + nom_tag).Data());
      TH2F* nom_hratedenom = (TH2F*)h_nom_pt_denom->Clone((nom_tag +"_denom").Data());
      nom_eff_rate->Add(h_nom_mcpt_num,-1.);
      nom_hratedenom->Add(h_nom_mcpt_denom, -1.);

      
      eff_rate->Divide(eff_rate,hratedenom,1.,1.,"cl=0.683 b(1,1) mode");
      nom_eff_rate->Divide(nom_eff_rate,nom_hratedenom,1.,1.,"cl=0.683 b(1,1) mode");

      
      TH2F* error_iso = (TH2F*)eff_rate->Clone(("MCerror_up_" + tag).Data());

      error_iso->Add(nom_eff_rate, -1.);
      error_iso->Divide(error_iso,eff_rate, 1.,1.,"cl=0.683 b(1,1) mode");
      error_iso->Write();
