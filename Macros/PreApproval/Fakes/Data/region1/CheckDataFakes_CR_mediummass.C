...
void CheckDataFakes_CR_mediummass(){
  
  TH1::SetDefaultSumw2(true);
  setTDRStyle();
  
  TString param = "HighMETBJetCR_MediumMassRegion_fakeparam_";
  ...
  for(unsigned int j= 0 ; j < jetpt.size() ; j++){
    ...
    if(j == 2){
      h_mc_presel->SetFillColor(kCyan);
    }
    if(j==3){
  ...
  c1->SaveAs(("/home/jalmond/WebPlots/PreApproval/DataFakes/Preselection" + param + "_looseregion1.pdf"));
