...
void MakeMCClosurePlot_Wjets(){
  ...
  collection.push_back("MCclosure/HNTight_loosereg2region2MCSSclosure");
  ...
  for(unsigned int k = 0 ; k < 1 ; k ++){
    ...
    double wjet_err(0.);
    float wjet_tt =   h_w_measured->IntegralAndError(0, h_w_measured->GetNbinsX(),wjet_err, "");
    
    
    TLine *line = new TLine( mcclosure_hist->GetBinLowEdge(mcclosure_hist->GetXaxis()->GetFirst()), h_w_measured->Integral(),mcclosure_hist->GetBinLowEdge(mcclosure_hist->GetXaxis()->GetLast()+1),h_w_measured->Integral());
  ...
  for(unsigned int j = 0; j < jetpt.size(); j++){
	...
	TH1F* h_wjet_p= (TH1F*)fwjet->Get((collection.at(k) + "_fake_predicted" +jetpt.at(j) + list_of_names.at(i)));
	double wjet_p_err;
	double int_wjet_p = h_wjet_p->IntegralAndError(0, h_wjet_p->GetNbinsX(), wjet_p_err, "");
	int bin = -9;
	...
	jy[bin] = int_wjet_p; jyerr[bin] = sqrt( (wjet_p_err*wjet_p_err) + ((int_wjet_p*0.35)*(int_wjet_p*0.35)));
	cout << int_wjet_p << " " << wjet_p_err << endl;
  ...
  c1->SaveAs(("/home/jalmond/WebPlots/PreApproval/MCclosure/WjetMCclosure_looseregion2_region2.pdf"));
