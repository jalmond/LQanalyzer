...
void MakeNsignalEff_tight(){
  
  setTDRStyle();
  gStyle->SetPalette(1);

  TH1* medium = makehist("PreSelection_medium");
  TH1* tight = makehist("PreSelection_tight");
  TH1* susy = makehist("PreSelection_susy");
  TH1* tight_dxy10 = makehist("PreSelection_iso_10_10");
  TH1* tight_anal = makehist("PreSelection");
  ...
  c1->SaveAs(("/home/jalmond/WebPlots/PreApproval/SignalPlots/SignalEff_presel_med_tight.pdf" ));
}
