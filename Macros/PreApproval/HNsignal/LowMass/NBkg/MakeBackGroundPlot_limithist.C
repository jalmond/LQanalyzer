...
void MakeBackGroundPlot_limithist(){
  
  setTDRStyle();
  gStyle->SetPalette(1);

  TH1F* tight_anal1 = makehist("limithist/40_fg_defaultMassRegion_limithist");
  cout << tight_anal1->GetBinContent(2);
...
TH1F* makehist(TString hist){
  ...
  TH1F* cf = makecfHist(hist, 600, 0);    
  TH1F* np = makenpHist(hist, 600, 0);    
  TH1F* mc = makemcHist(hist, 600, 0);    
