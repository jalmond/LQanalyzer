...
void NoteV2_unblind_limit_emu_crosssection(){
  ...
  TString outfile = "Limit_file_emu_crosssection.root";
  ...
  Double_t bkg[]   = {33.7,33.7,33.7,33.7,25.27,              20.05,  14.82,  12.302, 13.846, 9.28, 6.21, 4.74,  2.17 ,1.63, 1.56, 1.56};
  Double_t bkg_err[]  = {0.17, 0.17 , 0.17, 0.17 , 0.17 , 0.11 , 0.11 , 0.11 , 0.11 , 0.11  , 0.11  , 0.11    , 0.11  ,  0.11   , 0.11   , 0.11};
  Double_t fake[] = {30.6, 30.6,30.6,30.6,17.17 ,             13.39,  8.05709, 5.07276, 5.56404, 3.62671, 2.45944, 1.53942, 0.680018,  0.680018, 0.680018,0.685104, 0.685104};
  
  Double_t stat[] = {2.99496,2.99496,2.99496,2.99496, 1.91,   1.565, 1.18328, 1.12164, 1.13594, 0.855191, 0.703202, 0.605436, 0.425683, 0.382889, 0.390432,0.394941,0.390432};
  
  //Double_t sigeff_emu[] = {0.00401305, 0.00426041, 0.00358092, 0.00127962, 0.00561703, 0.00548436, 0.0160057, 0.0490417, 0.0858179, 0.0979121, 0.103817, 0.140004, 0.139994, 0.137478, 0.13651, 0.114804};
  //Double_t sigeff_mue[] = {0.00355829, 0.00465026, 0.00378657, 0.00141292, 0.00556388, 0.00537013, 0.0163115, 0.0473998, 0.0868511, 0.0974997, 0.107157, 0.136254, 0.139491, 0.139913, 0.137691, 0.11512};

  Double_t sigeff[] = {0.00394055, 0.0046411,0.00381043,0.0013734,0.00575442,0.00567942,0.0171124,0.0524423,0.0954012,0.108635,0.117892,0.156322,0.160046,0.161052,0.162037,  0.141089};
  ...
  for(unsigned int i=0; i < 16; i++){
    ...
    h_xsec->SetBinContent(i+1,2.*xsec_alp[i]);
    ...
    h_sigeff->SetBinContent(i+1, (sigeff[i]));
    eff_alp[i] = (sigeff[i]);
    ...
    float bkgerr = sqrt( pow(0.35*fake[i],2) + pow(mcerr* (mc),2)  +  pow(.12*(cf[i]),2)  + pow(staterr,2) );
    h_bkgerr->SetBinContent(i+1,bkgerr);
    

    /// Need re calculate thie because PDF uncertainty is wrong
    float err_sig_up = sqrt(sigeff_emu_err[i] *sigeff_emu_err[i] - q2[i]*q2[i] - 0.035*0.035 + 0.02*0.02);
    float err_sig_down = sqrt(sigeff_emu_err[i] *sigeff_emu_err[i] - q2[i]*q2[i] - 0.035*0.035 + 0.02*0.02);
