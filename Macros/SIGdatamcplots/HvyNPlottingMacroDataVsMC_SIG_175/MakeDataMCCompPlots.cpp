...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
	...
	TFile* file_sig40 =  TFile::Open(("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee175_nocut_5_3_14.root"));
...
TCanvas* CompDataMC(TH1* hdata, TH1* hsig_40, TH1* hsig_80, TH1* hsig_200, vector<THStack*> mcstack,TH1* hup, TH1* hdown,TLegend* legend, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  legend->AddEntry(hsig_40, "m_{N} = 175 GeV","l"); 
  legend->Draw("same");
