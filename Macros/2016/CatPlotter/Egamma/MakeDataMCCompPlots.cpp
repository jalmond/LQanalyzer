...
vector<pair<TString,float> >  InitSample (TString sample){
  ...
  if(sample.Contains("top")){   
    list.push_back(make_pair("TTJets_MG",0.2));
    ...
    list.push_back(make_pair("ttbb",0.2));
  }
  if(sample.Contains("vvv")){    list.push_back(make_pair("WWW",0.2));
    list.push_back(make_pair("ZZZ",0.2));
    list.push_back(make_pair("WZZ",0.2));
  }if(sample.Contains("vv")){    list.push_back(make_pair("WW",0.2));
  ...
  }if(sample.Contains("DYhigh")){    list.push_back(make_pair("DYJets",0.15));
  }  
...
if(name.find("h_Nmuons")!=string::npos) xtitle="Number of muons";
if(name.find("h_Nbjets")!=string::npos) xtitle="Number of b-jets";
...
vector<pair<TString,float> >  DYhigh = InitSample(" DYhigh"); 
  ...
  for( unsigned int i = 0; i < listofsamples.size(); i++){
    if(listofsamples.at(i) =="nonprompt_DoubleEG")samples.push_back(make_pair(make_pair(np,870),"Misid. Lepton Background"));
   if(listofsamples.at(i) =="higgs")samples.push_back(make_pair(make_pair(higgs,800),"Higgs"));
   if(listofsamples.at(i) =="top")samples.push_back(make_pair(make_pair(top,88),"t#bar{t},t/#bar{t},t/#bar{t}W,t#bar{t}V"));
   if(listofsamples.at(i) =="wjet")samples.push_back(make_pair(make_pair(wjet,92),"W+Jets"));
   if(listofsamples.at(i) =="vvv")samples.push_back(make_pair(make_pair(vvv,74),"Triboson"));
   if(listofsamples.at(i) =="vv")samples.push_back(make_pair(make_pair(vv,74),"Diboson"));
   if(listofsamples.at(i) =="DYlow")samples.push_back(make_pair(make_pair(DYlow,632),"DY#rightarrow ll; 10 < m(ll) < 50"));
   if(listofsamples.at(i) =="DYhigh")samples.push_back(make_pair(make_pair(DYhigh,64),"DY#rightarrow ll; m(ll) > 50"));
