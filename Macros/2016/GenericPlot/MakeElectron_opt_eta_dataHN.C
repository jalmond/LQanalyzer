...
void MakeElectron_opt_eta_dataHN(){
  ...
  TFile * fmc_s = new TFile( (path+"/HNDiElectron_data_DoubleEG_cat_v8-0-7.root" ).Data());

  TString hist1 = "DiElectron_mvatight_electron_eta_presel";
  ...
  bool norm=true;
  int rb=1;
  TH1F* hS = MakePlot(analyzer,date,"HNEpEp_40_cat","Hists/"+hist1,norm, fmc_s,rb); 
  TH1F* hS2 = MakePlot(analyzer,date,"HNEpEp_200_cat","Hists/"+hist2,norm, fmc_s,rb); 
  TH1F* hS3 = MakePlot(analyzer,date,"HNEpEp_1000_cat","Hists/"+hist3,norm, fmc_s,rb); 
  TH1F* hS4 = MakePlot(analyzer,date,"HNEpEp_1200_cat","Hists/"+hist4,norm, fmc_s,rb); 
  TH1F* hS5 = MakePlot(analyzer,date,"HNEpEp_1500_cat","Hists/"+hist5,norm, fmc_s,rb); 
    
    

  TString cantag= hist1;
  ...
  TLegend *legendS = new TLegend(1-(r+ 0.40), 1-(t + 0.2), 1- (r+ 0.1),  1-(t + 0.05));
  ...
  legendS->AddEntry(hS,"SS ee MVA (80%)","l");
  legendS->AddEntry(hS2,"SS ee HN (Tight)","l");
  legendS->AddEntry(hS3,"SS ee HN (Tight) (Relaxed IP)","l");
  legendS->AddEntry(hS4,"SS ee HN (Tight) (Relaxed dxy)","l");
  legendS->AddEntry(hS5,"SS ee HN (Tight) (Relaxed mva)","l");
  ...
  c1->SaveAs("HNDiLepton/"+hist1+"_dataHN.pdf","pdf");
  
  gSystem->Exec("scp HNDiLepton/"+hist1+"_dataHN.pdf jalmond@lxplus067.cern.ch:~/www/SignalHN/");
...
TH1F* MakePlot(TString analyzer, TString date,TString sample, TString hist, bool renorm, TFile* fmc, int rb){
  ...
  if(sample.Contains("_1200_")) h->SetLineColor(kCyan);
