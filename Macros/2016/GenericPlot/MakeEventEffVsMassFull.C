...
void MakeLeptonEffVsMassFull(){
  ...
  for( mit = masslist.begin(); mit !=masslist.end(); mit++,i++){
    ...
    float n_dijet= hS1->GetBinContent(10);
    float n_1jet= hS1->GetBinContent(11);
    float n_Fjet= hS1->GetBinContent(12);
    

    h1->SetBinContent(i,n_trigger/n_nocut);
    h2->SetBinContent(i,n_pt/n_nocut);
    h3->SetBinContent(i,n_pog/n_nocut);
    h4->SetBinContent(i,n_dxy/n_nocut);
    h5->SetBinContent(i,n_iso/n_nocut);
  ...
  h1->GetYaxis()->SetTitle("Culm:Breakdown of muon eff.");
  ...
  c1->SaveAs("HNDiLeptonHighMass/"+histname+"_full.pdf","pdf");
    
  gSystem->Exec("scp HNDiLeptonHighMass/"+histname+"_full.pdf jalmond@lxplus074.cern.ch:~/www/SignalHN/HighMass/");
  
  cout << "~/www/SignalHN/HighMass/"+histname+"_full.pdf" << endl;
}
