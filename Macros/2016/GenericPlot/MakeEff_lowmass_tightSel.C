...
void setTDRStyle();
TH1F* MakePlot(TString analyzer,TString sample, TString hist, TString cut, bool norm, TFile* f, int rb, TH1F* hdata);
float Get90Xval(TH1* h);

void MakeEff_lowmass_tightSel(){

  setTDRStyle();
  vector<TString> hists;
  hists.push_back("DiMuon_inclusive_pujetslowmass_tight_newbin");


  string indexname="HNDiLepton/LowMass_eff_index.html";
  ofstream ofile_tex;
  ...
  TFile * fbkg = new TFile( (path+"/HNDiLepton_BKG_hndilep_cat_v8-0-7.root").Data());
  ...
  for(unsigned int ihist= 0; ihist < hists.size() ; ihist++){
    ...
    TH1F* hdata = (TH1F*)fbkg->Get(dirname+hists[ihist]);

    TH1F* hS = MakePlot(analyzer,date,"HNMuMu_40_cat",dirname+hists[ihist],norm, fmc_s_40,rb, hdata); 
    TH1F* hS2 = MakePlot(analyzer,date,"HNMuMu_50_cat",dirname+hists[ihist],norm,fmc_s_50,rb,hdata); 
    TH1F* hS3 = MakePlot(analyzer,date,"HNMuMu_60_cat",dirname+hists[ihist],norm,fmc_s_60,rb,hdata); 
    TH1F* hS4 = MakePlot(analyzer,date,"HNMuMu_70_cat",dirname+hists[ihist],norm,fmc_s_70,rb,hdata); 
    TH1F* hS5 = MakePlot(analyzer,date,"HNMuMu_80_cat",dirname+hists[ihist],norm,fmc_s_80,rb,hdata); 
    ...
    legendS->AddEntry(hS3,"Schann. MM m_{N}(60)","l");
    legendS->AddEntry(hS4,"Schann. MM m_{N}(70)","l");
    legendS->AddEntry(hS5,"Schann. MM m_{N}(80)","l");
    ...
    gSystem->Exec("scp HNDiLepton/"+hists[ihist]+".pdf jalmond@lxplus067.cern.ch:~/www/SignalHN/");
...
TH1F* MakePlot(TString analyzer, TString date,TString sample, TString hist, bool renorm, TFile* fmc, int rb, TH1F* hdata){
  ...
  if(sample.Contains("40"))    sigeff=0.43;
  if(sample.Contains("50"))    sigeff=0.48;
  if(sample.Contains("60"))    sigeff=0.39;
  if(sample.Contains("70"))    sigeff=0.15;
  if(sample.Contains("80"))    sigeff=0.46;
  
  
  //cout << sample <<  " sig eff = " << sigeff << " " << bkg << " punzi ="  << sigeff / (1+ sqrt(bkg + (0.3*bkg)*(0.3*bkg))) << endl;
  cout << sample <<  " sig eff = " << sigeff << " " << bkg << " punzi ="  << sigeff / sqrt(bkg ) << endl;
  for(unsigned int i=1; i < h->GetNbinsX()+1; i++){
    cout << "Bin " << i << " = "<<  h->GetBinContent(i)/n << "   bkg = " <<  hdata->GetBinContent(i) <<  " " <<   h->GetBinContent(i)/(n*sqrt(hdata->GetBinContent(i) ))  <<endl;
    //cout << "punzi = " << 100*(h->GetBinContent(i)/n ) / (1+ sqrt(hdata->GetBinContent(i) + (0.3*hdata->GetBinContent(i))*(0.3*hdata->GetBinContent(i)))) << endl;
    if(hdata->GetBinContent(i) > 0)h->SetBinContent(i, h->GetBinContent(i)/ (sqrt(hdata->GetBinContent(i))));
    else h->SetBinContent(i, h->GetBinContent(i)/ (sqrt(1.8)));
  }
  if(renorm)h->Scale(1./n);
