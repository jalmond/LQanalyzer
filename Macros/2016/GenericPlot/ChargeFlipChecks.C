...
void ChargeFlipChecks(){

  setTDRStyle();
  vector<TString> hists;
  hists.push_back("BB_dxy_");
  hists.push_back("EE_dxy_");
  hists.push_back("BB_iso_");
  hists.push_back("EE_iso_");
  hists.push_back("BB_mva_");
  hists.push_back("EE_mva_");
  hists.push_back("BB_misshit_");
  hists.push_back("EE_misshit_");  
  hists.push_back("Mass_BB_");
  hists.push_back("Mass_EE_");

  string indexname="CF/CF_index.html";
  ofstream ofile_tex;
  ...
  TFile * fmc_s = new TFile( (path+"/"+analyzer+"_SKDYJets_hndilep_cat_v8-0-7.root" ).Data());
  ...
  for(unsigned int ihist= 0; ihist < hists.size() ; ihist++){
    ...
    TH1F* hSCF = MakePlot(analyzer,date,"SKDYJets",dirname+"_CF_"+hists[ihist],norm, fmc_s,rb); 
    TH1F* hZ = MakePlot(analyzer,date,"SKDYJets",dirname+"_Z_"+hists[ihist],norm, fmc_s,rb); 
    TH1F* hSEx = MakePlot(analyzer,date,"SKDYJets",dirname+"_ExC_"+hists[ihist],norm, fmc_s,rb); 
    TH1F* hSIn = MakePlot(analyzer,date,"SKDYJets",dirname+"_IcC_"+hists[ihist],norm, fmc_s,rb); 
    ...
    float maxS=hSCF->GetMaximum();
    if(hSIn->GetMaximum() > maxS) maxS = hSIn->GetMaximum() ;
    maxS=maxS*1.5;
    ...
    float x_range = Get90Xval(hSCF);
    x_range*= 1.3;
    cout << "x_range = " << x_range << endl;

    hSCF->GetYaxis()->SetRangeUser(0.,maxS);
    if(hists[ihist].Contains("jj"))     hSCF->GetXaxis()->SetRangeUser(10,x_range);
    if(hists[ihist].Contains("eta"))   hSCF->GetXaxis()->SetRangeUser(-1*x_range,x_range);
    

    hSCF->GetYaxis()->SetTitle("Events/bin");
    hSCF->Draw("hist");
    hZ->Draw("histsame");
    hSEx->Draw("histsame");
    hSIn->Draw("histsame");
    ...
    legendS->AddEntry(hZ,"Z-> ee (prompt)","l");
    legendS->AddEntry(hSCF,"Z-> ee (CF)","l");

    legendS->AddEntry(hSEx,"Z-> ee (Ex. Conv)","l");
    legendS->AddEntry(hSIn,"Z-> ee (In. Conv)","l");
    ...
    c1->SaveAs("CF/"+hists[ihist]+".pdf","pdf");
    
  }
     
    
  gSystem->Exec("scp -r CF/*  jalmond@lxplus024.cern.ch:~/www/SignalHN/");
...
TH1F* MakePlot(TString analyzer, TString date,TString sample, TString hist, bool renorm, TFile* fmc, int rb){
  ...
  if(hist.Contains("CF")) h->SetLineColor(kBlue);
  if(hist.Contains("Ex")) h->SetLineColor(kRed);
  if(hist.Contains("Ic")) h->SetLineColor(kGreen -2);
  if(hist.Contains("Ic")) h->SetLineStyle(2);
  if(hist.Contains("Z")) h->SetLineColor(kRed);
  if(hist.Contains("Z")) h->SetLineStyle(3);


  h->SetLineWidth(2.);
  h->Rebin(rb);
  if(!hist.Contains("Mass")){
