...
void MakeLeptonEffVsMassEJet(){
  ...
  for( mit = masslist.begin(); mit !=masslist.end(); mit++,i++){
    ...
    TH1F* h_stat = (TH1F*)fmc_->Get("Hists/DiElectron_hn_StatBin");
    ...
    */
    ...
    float n_lep= hS1->GetBinContent(13); 
    float n_dijet= hS1->GetBinContent(14);
    float n_jet= hS1->GetBinContent(15);
    float n_jet1= hS1->GetBinContent(16);
    float n_jet2= hS1->GetBinContent(17);
    float n_jet3= hS1->GetBinContent(18);
    ...
    float lm = 0.;
    if(i < 6)lm = hS1->GetBinContent(20) + hS1->GetBinContent(21);
    else lm = hS1->GetBinContent(21) ;
    float hm = 0.;
    if(i < 6)hm = hS1->GetBinContent(22);
    else hm = hS1->GetBinContent(22) + hS1->GetBinContent(23);
    ...
    float err1 = hS1->GetBinError(13)/n_nocut;
    if(err1 < 0.001) err1=0.001;
    float err2 = hS1->GetBinError(15)/n_nocut;
    if(err2 < 0.001) err2=0.001;
    float err3 = sqrt(hS1->GetBinError(20)* hS1->GetBinError(20) +  hS1->GetBinError(21)* hS1->GetBinError(21))/n_nocut;
    if(err3 < 0.001) err3=0.001;
    float err4 = sqrt(hS1->GetBinError(22)* hS1->GetBinError(22) +  hS1->GetBinError(23)* hS1->GetBinError(23))/n_nocut;
    if(err4 < 0.001) err4=0.001;
    cout <<     std::setprecision (2) << mit->second << " & " << nstat<< " & "  << n_lep/n_nocut << " $\\pm$ " <<  err1 << " & "  ;
    cout  <<     std::setprecision (2) << hS1->GetBinContent(15)/n_nocut  << " $\\pm$ " << err2;
    cout  <<     std::setprecision (2) << " & " << (hS1->GetBinContent(20)+hS1->GetBinContent(21))/n_nocut ;
    cout  <<     std::setprecision (2) << " $\\pm$ " << err3;
    cout  <<     std::setprecision (2) <<" & " << (hS1->GetBinContent(22)+hS1->GetBinContent(23))/n_nocut <<   " $\\pm$ " <<  err3  << "\\\\" << endl;
  ...
  h2b->SetLineWidth(2.0);
  h2c->SetLineWidth(2.0);
  ...
  h1->GetYaxis()->SetTitle("Selection eff. for ee channel.");
  ...
  label.DrawLatex(0.2 ,0.44,"S-channel");
  label.DrawLatex(0.8 ,0.44,"T-channel");
