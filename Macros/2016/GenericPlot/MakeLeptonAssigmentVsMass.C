...
void MakeLeptonAssigmentVsMass(){


  setTDRStyle();
  TString histname = "DiMuon_inclusive_Muon_From_N";
  ...
  for( mit = masslist.begin(); mit !=masslist.end(); mit++,i++){
    ...
    bool norm=true;
    int rb=1;
    TH1F* hS = MakePlot(analyzer,date,"HNMuMu_"+mit->second+"_cat",dirname+histname,norm, fmc_,rb); 
    
    float n1 = hS->GetBinContent(1);
    
    cout << mit->second << " " << n1 << endl;
    h->SetBinContent(i,n1);
  }
  h->SetMarkerStyle(23);
  h->SetLineWidth(2.);
  h->GetYaxis()->SetTitle("Fraction of lep_{1} from N");
  ...
  gSystem->Exec("scp HNDiLeptonHighMass/"+histname+"all.pdf jalmond@lxplus022.cern.ch:~/www/SignalHN/HighMass/");
