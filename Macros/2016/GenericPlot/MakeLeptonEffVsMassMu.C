...
void MakeLeptonEffVsMassMu(){
  ...
  for( mit = masslist.begin(); mit !=masslist.end(); mit++,i++){
    ...
    */
    ...
    float n_pog= hS1->GetBinContent(3);
    float n_pt= hS1->GetBinContent(4);
    float n_dxy= hS1->GetBinContent(5);
    float n_dz= hS1->GetBinContent(6);
    float n_dxysig= hS1->GetBinContent(7);
    float n_iso= hS1->GetBinContent(8);

    h1->SetBinContent(i,n_gen/n_nocut);
    h2->SetBinContent(i,n_pt/n_gen);
    h3->SetBinContent(i,n_pog/n_pt);
    h4->SetBinContent(i,n_dxy/n_pog);
    h5->SetBinContent(i,n_dz/n_dxy);
    h6->SetBinContent(i,n_dxysig/n_dz);
    h7->SetBinContent(i,n_iso/n_dxysig);
  ...
  legend->AddEntry(h3,"Cut 3:POG Tight","l");
  legend->AddEntry(h4,"Cut 3:d_{xy}","l");
  legend->AddEntry(h5,"Cut 4:d_{z}","l");
  legend->AddEntry(h6,"Cut 3:IP_{3D}","l");
  ...
  gSystem->Exec("scp "+histname+".pdf jalmond@lxplus022.cern.ch:~/www/SignalHN/");
  
  cout << "~/www/SignalHN/"+histname+".pdf" << endl;
}
