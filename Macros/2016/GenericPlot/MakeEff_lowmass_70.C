...
void MakeEff_lowmass_70(){

  setTDRStyle();
  vector<TString> hists;
  hists.push_back("DiMuon_inclusive_pujetsll_lowmasssig_binX");
  ...
  TFile * fbkg = new TFile( (path+"/HNDiLepton_data_DoubleMuon_cat_v8-0-7.root").Data());
...
TH1F* MakePlot(TString analyzer, TString date,TString sample, TString hist, bool renorm, TFile* fmc, int rb, TH1F* hdata){
  ...
  cout << sample <<  " punzi = " << sigeff / (1+ sqrt(bkg + (0.3*bkg)*(0.3*bkg))) << endl;
  for(unsigned int i=1; i < h->GetNbinsX()+1; i++){
    cout << "Bin " << i << " = "<<  h->GetBinContent(i)/n << "   bkg = " <<  hdata->GetBinContent(i) << endl;
