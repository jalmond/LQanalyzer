...
void MakeElectron_opt_etaHN(){
  ...
  TString hist = "DiElectron_mvatight_electron_eta_presel";
  TString hist2 = "DiElectron_hn_electron_eta_presel";
  TString hist3 = "DiElectron_hn_looseip_electron_eta_presel";
  TString hist4 = "DiElectron_hn_looseip2_electron_eta_presel";
  TString hist5 = "DiElectron_hn_loosemva_electron_eta_presel";


  TFile * fmc_s_100 = new TFile( (path+"/"+analyzer+"_HNEpEp_100_cat_v8-0-7.root" ).Data());


  bool norm=true;
  int rb=1;
  TH1F* hS = MakePlot(analyzer,date,"HNEpEp_100_cat","Hists/"+hist,norm, fmc_s_100,rb); 
  TH1F* hS2 = MakePlot(analyzer,date,"HNEpEp_200_cat","Hists/"+hist2,norm, fmc_s_100,rb); 
  TH1F* hS3 = MakePlot(analyzer,date,"HNEpEp_300_cat","Hists/"+hist3,norm, fmc_s_100,rb); 
  TH1F* hS4 = MakePlot(analyzer,date,"HNEpEp_400_cat","Hists/"+hist4,norm, fmc_s_100,rb); 
  TH1F* hS5 = MakePlot(analyzer,date,"HNEpEp_500_cat","Hists/"+hist5,norm, fmc_s_100,rb); 
  ...
  TLegend *legendS = new TLegend(1-(r+ 0.4), 1-(t + 0.3), 1- (r+ 0.1),  1-(t + 0.1));
  ...
  hS->GetYaxis()->SetRangeUser(0.,0.15);
  ...
  legendS->AddEntry(hS,"m_{N} 100:  MVA (80%)","l");
  legendS->AddEntry(hS2,"m_{N} 100: HN (Tight)","l");
  legendS->AddEntry(hS3,"m_{N} 100: HN (Tight) (Relaxed IP)","l");
  legendS->AddEntry(hS4,"m_{N} 100: HN (Tight) (Relaxed dxy)","l");
  legendS->AddEntry(hS5,"m_{N} 100: HN (Tight) (Relaxed mva)","l");
  ...
  latex.DrawLatex((r+ 0.15), 1-(t + 0.05), "Relaxed IP cuts:");
  latex.DrawLatex((r+ 0.15), 1-(t + 0.1), "- d(xy) < 0.02/0.05 (B/EC)");
  latex.DrawLatex((r+ 0.15), 1-(t + 0.15), "- d_(z) < 0.1/0.2 (B/EC)");

  c1->SaveAs("HNDiLepton/"+hist+"HN.pdf","pdf");
  
  gSystem->Exec("scp HNDiLepton/"+hist+"HN.pdf jalmond@lxplus067.cern.ch:~/www/SignalHN/");
...
TH1F* MakePlot(TString analyzer, TString date,TString sample, TString hist, bool renorm, TFile* fmc, int rb){
  ...
  if(sample.Contains("_400_")) h->SetLineColor(kBlue);
  if(sample.Contains("_200_")) h->SetLineColor(kRed);
  if(sample.Contains("_100_")) h->SetLineColor(kGreen -2);
  if(sample.Contains("_300_")) h->SetLineColor(kCyan);
  if(sample.Contains("_400_")) h->SetLineColor(kOrange);
