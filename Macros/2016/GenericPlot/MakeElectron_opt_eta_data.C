...
void MakeElectron_opt_eta_data(){
  ...
  TString hist2 = "DiElectron_pogtight_cc_electron_eta_presel";
  TString hist3 = "DiElectron_pogmedium_cc_electron_eta_presel";
  bool norm=true;
  int rb=1;
  ...
  TLegend *legendS = new TLegend(1-(r+ 0.35), 1-(t + 0.2), 1- (r+ 0.05),  1-(t + 0.05));
  ...
  legendS->AddEntry(hS2,"SS ee POG CB(Tight)","l");
  legendS->AddEntry(hS3,"SS ee POG CB(Medium)","l");
  ...
  latex.DrawLatex((r+ 0.15), 1-(t + 0.1), "- d(xy) < 0.05/0.1 (B/EC)");
  latex.DrawLatex((r+ 0.15), 1-(t + 0.15), "- d_(z) < 0.1/0.2 (B/EC)");
  latex.DrawLatex((r+ 0.15), 1-(t + 0.2), "- tight-charge");
  latex.DrawLatex((r+ 0.15), 1-(t + 0.25), "- rel_iso < 0.05 (MVA only)");
  legendS->Draw();
  
  c1->SaveAs("HNDiLepton/"+hist1+"_data.pdf","pdf");
  
  gSystem->Exec("scp HNDiLepton/"+hist1+"_data.pdf jalmond@lxplus067.cern.ch:~/www/SignalHN/");
