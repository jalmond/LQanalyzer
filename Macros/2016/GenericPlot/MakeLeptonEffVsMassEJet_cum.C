...
void MakeLeptonEffVsMassEJet(){
  ...
  for( mit = masslist.begin(); mit !=masslist.end(); mit++,i++){
    ...
    */
    ...
    h2a->SetBinContent(i,n_dijet/n_nocut);
  ...
  for( mit = masslistT.begin(); mit !=masslistT.end(); mit++,i++){
    ...
    bool norm=true;
    int rb=1;
    cout << path+"/"+analyzer+"_HNDilepton_EE_Tchannel_M"+mit->second+"_cat_v8-0-7.root" << endl;
  ...
  legend->AddEntry(h2,"SS2l+(j+2j)","l");
  ...
  legend2->AddEntry(h3,"#frac{SS2l+(j+2j)}{SS2l+2j}","l");
  legend2->Draw();
