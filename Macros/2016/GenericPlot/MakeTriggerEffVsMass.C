...
void MakeTriggerEffVsMass(){


  setTDRStyle();
  TString histname = "DiMuon_inclusivell_lowmass_flow";
  ...
  for( mit = masslist.begin(); mit !=masslist.end(); mit++,i++){
    ...
    float n_dilep= hS1->GetBinContent(3);
    float n_trigger= hS1->GetBinContent(4);
    

    h1->SetBinContent(i,n_trigger/n_dilep);
  ...
  h1->GetYaxis()->SetTitle("Trigger efficiency wrt dilepton selection");
  ...
  c1->SaveAs("HNDiLeptonHighMass/Trigger.pdf","pdf");
    
  gSystem->Exec("scp HNDiLeptonHighMass/Trigger.pdf jalmond@lxplus102.cern.ch:~/www/SignalHN/HighMass/");
