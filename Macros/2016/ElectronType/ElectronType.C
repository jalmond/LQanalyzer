...
void ElectronType(){
  ...
  for(int s=0; s < samples.size(); s++){
    ...
    for (int pl =0 ; pl < etypes.size(); pl++){
      ...
      makehist(etypes[pl] + sample +"_dxy", etypes[pl], path,  sample, "_dxy" , "dxy");
      makehist(etypes[pl] + sample +"_iso", etypes[pl], path,  sample, "_iso" , "iso");
      return;
      
      TFile * fmc = new TFile(path+"ElectronTypes_"+sample+"_cat_v8-0-6.root");
      TH1F* h_loose_pt= (TH1F*)fmc->Get((etypes[pl] + "_loose_pt"));
      TH1F* h_pt= (TH1F*)fmc->Get((etypes[pl] + "_pt"));

      if(!h_pt) continue;
      if(!h_loose_pt) continue;

      
      TH1F* h_tight_pt= (TH1F*)fmc->Get((etypes[pl] + "_tight_pt"));
      if(!h_tight_pt) continue;      
      
      TCanvas* c1 = new TCanvas("Plot"+etypes[pl] + sample, "Plot"+etypes[pl] + sample, 800, 600);
      

      h_loose_pt->Divide(h_pt);
      h_tight_pt->Divide(h_pt);
      h_loose_pt->GetXaxis()->SetTitle("p{T} (GeV)");
      h_loose_pt->GetXaxis()->SetTitle("p{T} (GeV)");
      

      h_loose_pt->Draw("hist");
      h_tight_pt->Draw("histsame");
      ...
      label.DrawLatex(0.6 ,0.7,sample);
      label.DrawLatex(0.5 ,0.7,etypes[pl]);

      c1->SaveAs("~/ElectronTypes/" + etypes[pl] + sample+"_FR.pdf");
