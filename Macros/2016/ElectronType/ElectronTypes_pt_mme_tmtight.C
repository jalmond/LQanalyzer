...
void ElectronsType_pt(){
  ...
  for(int s=0; s < samples.size(); s++){
    ...
    TH1F* h_pt= (TH1F*)fmc->Get("MME_tmtight_particletype");
    //TH1F* h_pt= (TH1F*)fmc->Get("SS__tmtight_particletype");
    
    TCanvas* c1 = new TCanvas("Plot", "Plot" , 800, 600);
    
    if(!h_pt) continue;
    h_pt->Draw("colzText");
    c1->SaveAs("~/ElectronTypes/MME_"+sample+"_particletype_tmtight.pdf");
  }
}
