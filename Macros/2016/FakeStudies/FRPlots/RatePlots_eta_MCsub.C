...
void FR_pt(){
  ...
  setTDRStyle();
  TString cut="eta";
  TH1F* h = MakeFRPlot("mc","ElELECTRON16_FR_POG_MEDIUM_CC_dijet_pog_40_",cut);
  TH1F* h2 = MakeFRPlot("mc","ElELECTRON16_FR_POG_TIGHT_CC_dijet_pog_40_",cut);
  TH1F* h3 = MakeFRPlot("mc","ElELECTRON16_FR_MVA_TIGHT_CC_dijet_mva_40_",cut);
  ...
  c1->SaveAs("TL_mcsub_pog_eta_id.pdf");
}
...
TH1D* MakePRPlot(TString sample, TString hist){
  ...
  TFile * fmc ;
  if(sample!=""){
    fmc= new TFile((path+"/FakeRateCalculator_El_mc_v8-0-6.root"));
...
TH1F* MakeFRPlot(TString sample, TString hist,TString cut){
  ...
  TH1F* hmcloose = (TH1F*)fmc->Get(("Loose"+hist+cut).Data());
  TH1F* hmctight = (TH1F*)fmc->Get("Tight"+hist+cut);
