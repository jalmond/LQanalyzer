...
void MakeFRRootFile(){
  ...
  TString path= "/data2/CAT_SKTreeOutput/JobOutPut/jalmond/LQanalyzer//data/output/CAT/FakeRateMC/periodBtoH/FakeRateMC_SKQCD_cat_v8-0-7.root";
  ...
  TString outfile = "FakeRate13TeV_El_2016_mcdilep.root";
  ...
  fakes40.push_back("_el_cj_pt");
  fakes40.push_back("_el_cj_ptcorr");
  fakes40.push_back("_el_ncj_pt");
  fakes40.push_back("_el_ncj_ptcorr");
  ...
  for(vector<TString>::iterator it = ids.begin(); it!=ids.end(); ++it){
    for(vector<TString>::iterator it2 = fakes40.begin(); it2!=fakes40.end(); ++it2){
      ...
      TString denom =*it+"Loose"  +  *it2;
      TString num =*it +"Tight"  + *it2;
      ...
      CheckHist(h_pt_num);
      cout << "Integral =" << h_pt_num->Integral() << endl;
      TString name =*it +*it2 ;
      TH2D* eff_rate = (TH2D*)h_pt_num->Clone(("FakeRate" + name).Data());
