...
def  MakeHistFile(histlist, binlist,xminlist, xmaxlist,jobname):
    ...
    if not os.path.exists(str(os.getenv("LQANALYZER_DIR")) + "/Macros/CatPlotterNoData/"+jobname ):
        os.system("mkdir " + str(os.getenv("LQANALYZER_DIR")) + "/Macros/CatPlotterNoData/"+jobname)
    ...
    file_cuts=open(str(os.getenv("LQANALYZER_DIR")) + "/Macros/CatPlotterNoData/"+jobname +"/dat/histfile.txt","w")
...
for line in input_configfile:
    ...
    if "cut " in line and "# " in line:
        ...
        sline = line.split()
        inputfile = str(os.getenv("LQANALYZER_DIR")) + "/Macros/CatPlotterNoData/PlotConfig/"+sline[2]
        ...
        os.system('python  ' + os.getenv("LQANALYZER_DIR") + '/Macros/CatPlotterNoData/setupplotter.py -i ' + inputfile + ' -d ' + inputdir + ' -x ' + jobdir + ' -s ' + stream + ' -a ' + analyzer + ' -S ' + skim + ' -p ' + periodtag + ' -C ' + cutlist[0] + ' -M ' + configinputfile + ' -c ' + plottag)
        histlist=[]
        binlist=[]
        xminlist=[]
        xmaxlist=[]
