...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
	...
	map<TString,TH1*>::iterator dit = mhist.find("Nominal");
	TH1* hdata = MakeDataHist(name, xmin, xmax, hup, ylog, rebin);//dynamic_cast<TH1*> (dit->second->Clone(("data"+name).c_str()));
	//MakeSumHist(mstack);
...
TH1* MakeDataHist(string name, double xmin, double xmax, TH1* hup, bool ylog, int rebin){
  ...
  for(unsigned int i=0; i < hdata->GetNbinsX()+1; i++){
