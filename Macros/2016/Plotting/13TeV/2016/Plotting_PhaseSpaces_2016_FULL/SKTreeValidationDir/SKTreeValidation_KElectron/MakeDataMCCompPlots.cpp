...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
    ...
    for(unsigned int ncut=0; ncut<allcuts.size();  ncut++){
      string name = allcuts.at(ncut) + "/" + h_name+  allcuts.at(ncut);
      bool isSS(false);
      if(TString(allcuts.at(ncut)).Contains("SS"))isSS = true;

      cout << "###################### " << name << " ###########################"<< endl;
      
      TString fileloc =   mcloc;
      TFile* file =  TFile::Open((fileloc+ fileprefix + "WW_pythia8" + filepostfix).Data());
      cout << fileloc+ fileprefix + "WW_pythia8" + filepostfix << endl;
      cout << file << endl;
      ...
      h_tmp->SetLineWidth(2.5);

      TCanvas* c = CompDataMC(h_tmp,h_name+  allcuts.at(ncut),rebin,xmin,xmax,0.1,10000., path, histdir,ylog, showdata, channel);      	
      
      string canvasname = h_name+  allcuts.at(ncut);

      cout << "canvasname.erase(0,4) = " << canvasname << endl;
      cout << c->GetName() << endl;
      PrintCanvas(c, histdir, canvasname, canvasname);
    }
  }            
  page.close();
...
TCanvas* CompDataMC(TH1* h_tmp, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  cout << "cname+ label_plot_type = " << cname+ label_plot_type << endl;
