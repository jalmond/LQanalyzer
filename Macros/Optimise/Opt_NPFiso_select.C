...
void Opt_d0cut(){
  ...
  TH1F* h_npcutflow= (TH1F*)fnp->Get(("ISO_NPFISO_EEEB_cutflow"));
  TH1F* h_cfcutflow= (TH1F*)fcf->Get(("ISO_NPFISO_EEEB_cutflow"));
  TH1F* h_mccutflow= (TH1F*)fmc->Get(("ISO_NPFISO_EEEB_cutflow"));
  ...
  for(unsigned int im=0; im < masses.size(); im++){
    ...
    TH1F* h_cutflow= (TH1F*)fsig->Get(("ISO_NPFISO_EEEB_cutflow"));
    ...
    h_mass_d0opt->GetYaxis()->SetRangeUser(0.001, 0.5);
  ...
  legend->Draw("same");
  c1->SaveAs(("/home/jalmond/WebPlots/Opt/iso_npfiso_eb_optimise.pdf"));
}
