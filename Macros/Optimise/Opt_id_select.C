...
void Opt_idcut(){
  ...
  for(unsigned int im=0; im < masses.size(); im++){
    ...
    TH1F* h_mass_d0opt = new TH1F(("IDcut_opt_HN"+ masses.at(im)).Data(),("IDcut_opt_HN"+ masses.at(im)).Data(), 2,0.,2.);
    ...
    for(int i= 1; i < h_mass_d0opt->GetNbinsX()+1;i++){
      cout << "test " << i << endl;
      cout << h_cutflow << endl;
      float sig_eff = h_cutflow->GetBinContent(i) / 100000.;
      cout << h_npcutflow << " " << h_cfcutflow << " " << h_mccutflow << endl;
  ...
  legend->Draw("same");
  c1->SaveAs(("/home/jalmond/WebPlots/Opt/IDoptimise.pdf"));
}
