...
void Opt_ee(){
  ...
  TH1F* h_npcutflow= (TH1F*)fnp->Get(("SSee_DiJet/h_MET_SSee_DiJet"));
  TH1F* h_cfcutflow= (TH1F*)fcf->Get(("SSee_DiJet/h_MET_SSee_DiJet"));
  TH1F* h_mccutflow= (TH1F*)fmc->Get(("SSee_DiJet/h_MET_SSee_DiJet"));
  ...
  for(unsigned int imass=0; imass < masses.size(); imass++){
    ...
    TH1* hsig =  (TH1F*)fsig->Get(("SSee_DiJet/h_MET_SSee_DiJet"));
  ...
  for(std::map<TString, TH1*>::iterator it = histmap.begin(); it != histmap.end(); it++, i2++){
    ...
    for(int i=  h_npcutflow->GetNbinsX(); i > 0;  i--){
      ...
      float tot_bkg =  h_npcutflow->Integral(1,i) + h_mccutflow->Integral(1,i);
    ...
    cout << it->first << "  cut = " << h_npcutflow->GetBinLowEdge(iopt+1) << "  sigeff = " << opt_sig <<  " bkg = " << optbkg << endl;
  }
}
