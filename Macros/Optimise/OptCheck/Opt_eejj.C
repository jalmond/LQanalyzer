...
void Opt_eejj(){
  ...
  TH1F* h_npcutflow= (TH1F*)fnp->Get(("LowMassRegion_mvajet/h_eejjmass_LowMassRegion_mvajet"));
  TH1F* h_cfcutflow= (TH1F*)fcf->Get(("LowMassRegion_mvajet/h_eejjmass_LowMassRegion_mvajet"));
  TH1F* h_mccutflow= (TH1F*)fmc->Get(("LowMassRegion_mvajet/h_eejjmass_LowMassRegion_mvajet"));
  ...
  for(unsigned int imass=0; imass < masses.size(); imass++){
    ...
    TH1* hsig =  (TH1F*)fsig->Get(("LowMassRegion_mvajet/h_eejjmass_LowMassRegion_mvajet"));
  ...
  for(std::map<TString, TH1*>::iterator it = histmap.begin(); it != histmap.end(); it++, i2++){
    ...
    //legend->Draw("same");
    c1->SaveAs(("/home/jalmond/WebPlots/Opt/meejj_optimise_" + it->first + ".pdf").Data());
