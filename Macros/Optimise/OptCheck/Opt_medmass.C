...
void Opt_mT(){
  ...
  TH1F* h_npcutflow= (TH1F*)fnp->Get(("MidMassOptimise"));
  TH1F* h_cfcutflow= (TH1F*)fcf->Get(("MidMassOptimise"));
  TH1F* h_mccutflow= (TH1F*)fmc->Get(("MidMassOptimise"));
  TH1F* h_datacutflow= (TH1F*)fdata->Get(("MidMassOptimise"));
  ...
  for(unsigned int imass=0; imass < masses.size(); imass++){
    ...
    TH1* hsig =  (TH1F*)fsig->Get(("MidMassOptimise_sig"));
  ...
  for(std::map<TString, TH1*>::iterator it = histmap.begin(); it != histmap.end(); it++, i2++){
    ...
    //legend->Draw("same");
    c1->SaveAs(("/home/jalmond/WebPlots/Opt/MidMassoptimise_" + it->first + ".pdf").Data());
