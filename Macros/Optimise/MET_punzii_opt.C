...
//void MET_punzii_opt(){
{
  ...
  for(int im = 0 ; im < imasses ; im++){
    ...
    cout << "Opt cut : MET <  " << xaxis[imet_opt] << endl;
  ...
  frame->SetStats(0);
  frame->GetXaxis()->SetTitle("E^{miss}_{T} (GeV)");
  ...
  c1->SaveAs("/home/jalmond/WebPlots/Opt/Punzi/MET_punzi_optimisation.pdf");
}
