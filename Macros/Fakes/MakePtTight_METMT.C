...
void MakePtTight(TString path) {
  ...
  vector<TString> cuts;
  cuts.push_back("vloose");
  cuts.push_back("loose");
  cuts.push_back("medium");
  cuts.push_back("tight");
  
  for(std::vector<TString>::iterator it = cuts.begin(); it != cuts.end(); it++){
    
    TCanvas* c1 = new TCanvas(("Plot"+*it).Data(), ("Plot"+*it).Data(), 800, 600);
    TString cut = "SingleTightElJet_metmt_"+*it;

    /// Make Ptelectron_SingleTightElJet plot
    TH1F* h_data= (TH1F*)fdata->Get((cut + "/h_leadingElectronPt_" + cut).Data());
    TH1F* h_mc= (TH1F*)fmc->Get((cut + "/h_leadingElectronPt_" + cut).Data());
    ...
    TPad *p = new TPad( ("p_test"+*it).Data(), "", 0, 0, 1, 1.0 - SUBFIGURE_MARGIN, 0, 0, 0);  // create new pad, fullsize to have equal font-sizes in both plots
    ...
    for(int i=1; i < h_ratio->GetNbinsX()+1; i++){
      ...
      if((tot_p)!=0) h_ratio->SetBinContent(i, 100.* tot_p / (tot_np+tot_p) );
    ...
    c1->SaveAs(("/home/jalmond/WebPlots/Fakes/PT_tightsample_METMT"+ *it + ".pdf").Data());
  }
