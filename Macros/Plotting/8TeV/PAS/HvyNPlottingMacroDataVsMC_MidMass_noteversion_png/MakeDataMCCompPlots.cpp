...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
	...
	TFile* file_sig40 =  TFile::Open(("/home/jalmond/HeavyNeutrino/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee100_nocut_5_3_14.root"));
	...
	TFile* file_sig80 =  TFile::Open(("/home/jalmond/HeavyNeutrino/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee300_nocut_5_3_14.root"));
...
void PrintCanvas(TCanvas* c1, string folder, string plot_description, string title){
  ...
  std::string tpng = "/home/jalmond/WebPlots/"+ path +  "/histograms/"+folder+"/"+title;
...
void SetTitles(TH1* hist, string name){
  ...
  if(name.find("jets_pt")!=string::npos)xtitle="Jet p_{T} (GeV/c)";
  ...
  if(name.find("thirdELectronPt")!=string::npos)xtitle="Third electron p_{T} (GeV/c)";
  ...
  if(name.find("emumass")!=string::npos)xtitle="e^{#pm}mu^{#mp} invariant mass (GeV/c^{2})";
...
float  GetMaximum(TH1* h_data, TH1* h_up, bool ylog, string name){
  ...
  if(name.find("e2jj")!=string::npos) yscale*=1.1;
  if(name.find("charge")!=string::npos) yscale*=1.5;
  if(name.find("deltaR")!=string::npos) yscale*=1.5;
  if(name.find("bTag")!=string::npos) yscale*=2.2;
  if(name.find("eejj")!=string::npos) yscale*=1.07;
  if(name.find("dijetsmass")!=string::npos) yscale*=1.5;
  if(name.find("leadingElectronPt")!=string::npos) yscale*=1.2;
...
TCanvas* CompDataMC(TH1* hdata, TH1* hsig_40, TH1* hsig_80, vector<THStack*> mcstack,TH1* hup, TH1* hdown,TH1* hup_nostat,TLegend* legend, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  std::string tpng = "/home/jalmond/WebPlots/"+ path + "/histograms/"+folder+"/"+title+".png";
  std::string tlogpng = "/home/jalmond/WebPlots/"+ path + "/histograms/"+folder+"/"+title+"_log.png";
