...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
	...
	TFile* file_sig80 =  TFile::Open(("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee100_nocut_5_3_14.root"));

        TH1* hsig_80 = dynamic_cast<TH1*> ((file_sig80->Get(name.c_str()))->Clone());

	FixOverUnderFlows(hsig_80, xmax);
        SetTitles(hsig_80, name);
        ymax = GetMaximum(hsig_80, hsig_80, ylog, name);
        float int_bkg = hup->Integral()/2.;
	hsig_80->Rebin(rebin);
	FixOverUnderFlows(hsig_80, xmax);
	ymax = hsig_80->GetMaximum();

	hsig_80->Scale(1./hsig_80->Integral());
	hsig_80->Scale(int_bkg);
        hsig_80->GetYaxis()->SetRangeUser(ymin,30.);
	hsig_80->SetLineColor(kRed);
	hsig_80->SetLineWidth(2.);

	TFile* file_sig100 =  TFile::Open(("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee300_nocut_5_3_14.root"));
	...
	TFile* file_sig150 =  TFile::Open(("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee150_nocut_5_3_14.root"));
        ...
        hsig_150->SetLineColor(kMagenta+1);
	...
	TCanvas* c = CompDataMC(hdata,hsig_500,hsig_80,hsig_100, hsig_150, hsig_200,  hsig_300,  hsig_400 , hsig_500,vstack,hup,hdown, legend,name,rebin,xmin,xmax, ymin,ymax, path, histdir,ylog, showdata, channel);      	
...
void  SetUpConfig(vector<pair<pair<vector<pair<TString,float> >, int >, TString > >& samples, vector<string>& cut_label){
  ...
  for( unsigned int i = 0; i < listofsamples.size(); i++){
    ...
    if(listofsamples.at(i) =="prompt")samples.push_back(make_pair(make_pair(prompt,vvcol),"Prompt"));
    ...
    if(listofsamples.at(i) =="nonprompt")samples.push_back(make_pair(make_pair(np,fcol),"Misid. Muon Bkgnd"));   
...
TCanvas* CompDataMC(TH1* hdata, TH1* hsig_40, TH1* hsig_80, TH1* hsig_100,TH1* hsig_150,TH1* hsig_200, TH1* hsig_300,TH1* hsig_400,TH1* hsig_500, vector<THStack*> mcstack,TH1* hup, TH1* hdown,TLegend* legend, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  legend->AddEntry(hsig_80, "m_{N} = 100 GeV","l"); 
  legend->AddEntry(hsig_100, "m_{N} = 300 GeV","l"); 
