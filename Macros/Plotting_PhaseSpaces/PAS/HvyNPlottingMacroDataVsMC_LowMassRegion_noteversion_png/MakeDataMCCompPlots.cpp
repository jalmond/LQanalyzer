...
int MakePlots(string hist) {
  ...
  while(!histo_name_file.eof()) {
	...
	TFile* file_sig40 =  TFile::Open(("/home/jalmond/HeavyNeutrino/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee40_nocut_5_3_14.root"));
	...
	TFile* file_sig80 =  TFile::Open(("/home/jalmond/HeavyNeutrino/Analysis/LQanalyzer/data/output/SSElectron/HNDiElectron_SKHNee80_nocut_5_3_14.root"));
...
float  GetMaximum(TH1* h_data, TH1* h_up, bool ylog, string name){
  ...
  if(name.find("e2jj")!=string::npos) yscale*=1.5;
...
TCanvas* CompDataMC(TH1* hdata, TH1* hsig_40, TH1* hsig_80, vector<THStack*> mcstack,TH1* hup, TH1* hdown,TH1* hup_nostat,TLegend* legend, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  float ymax_err = 1.8;
  if(TString(hname).Contains("eejj")) ymax_err = 0.;
  ...
  legend->AddEntry(hsig_40, "m_{N} = 40 GeV/c^{2}, |V_{eN}|^{2} = 4#times10^{-4} ","l");
  legend->AddEntry(hsig_80, "m_{N} = 80 GeV/c^{2}, |V_{eN}|^{2} = 4#times10^{-3}","l");
