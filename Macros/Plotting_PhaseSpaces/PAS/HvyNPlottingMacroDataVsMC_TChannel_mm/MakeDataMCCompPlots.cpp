...
vector<pair<TString,float> >  InitSample (TString sample){
  ...
  if(sample.Contains("nonprompt")){
    list.push_back(make_pair("nonprompt",0.28));
  }
...
void SetTitles(TH1* hist, string name){
  ...
  if(name.find("l1jj_central_mass")!=string::npos)xtitle="l_{1}jj (central jets) invariant mass (GeV)";
  if(name.find("l2jj_central_mass")!=string::npos)xtitle="l_{2}jj (central jets) invariant mass (GeV)";
  if(name.find("lljj_central_mass")!=string::npos)xtitle="lljj (central jets) invariant mass (GeV)";
...
float  GetMaximum(TH1* h_data, TH1* h_up, bool ylog, string name){
  ...
  yscale*=1.2;
  if(name.find("Njets")!=string::npos) yscale*=3.;
  ...
  if(name.find("ElectronPt")!=string::npos) yscale*=2.;
  if(name.find("Nbje")!=string::npos) yscale*=2.;
  if(name.find("electrons_eta")!=string::npos) yscale*=1.5;
  if(name.find("nVert")!=string::npos) yscale*=4.;
...
void  SetUpConfig(vector<pair<pair<vector<pair<TString,float> >, int >, TString > >& samples, vector<string>& cut_label){
  ...
  np.push_back(make_pair("nonprompt",0.28));
...
TCanvas* CompDataMC(TH1* hdata, vector<THStack*> mcstack,TH1* hup, TH1* hdown,TH1* hup_nostat,TLegend* legend, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  label.DrawLatex(0.6 ,0.45,"T-Channel #mu^{#pm} #mu^{#pm} channel");
