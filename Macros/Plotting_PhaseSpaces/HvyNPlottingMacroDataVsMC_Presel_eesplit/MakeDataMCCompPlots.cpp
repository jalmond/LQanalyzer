...
TCanvas* CompDataMC(TH1* hdata, vector<THStack*> mcstack,TH1* hup, TH1* hdown,TH1* hup_nostat,TLegend* legend, const string hname, const  int rebin, double xmin, double xmax,double ymin, double ymax,string path , string folder, bool logy, bool usedata, TString channel) {
  ...
  if(usedata){
    ...
    g->SetLineWidth(2.0);    g->SetMarkerSize(0.);
    g->Draw( "p9same");
  ...
  if(usedata){
    ...
    for (int i = 0; i < g->GetN(); ++i) {
      ...
      double U =  (N==0) ?  ( ROOT::Math::gamma_quantile_c(alpha,N+1,1) ) :
	( ROOT::Math::gamma_quantile_c(alpha/2,N+1,1) );
      cout << "Data " << i << " = " << N << " up = " << U - N << " down = " << N - L <<  endl;
      if ( N!=0 ) {
	g->SetPointEYlow(i, N-L );
	g->SetPointEXlow(i, 0);
	g->SetPointEYhigh(i, U-N );
	...
	hdev_err_stat->SetBinContent(i, 1.0);
      ...
      vector<float> yval;
      vector<float> yval_up;
      vector<float> yval_down;


      for (Int_t i=1;i<=hdev->GetNbinsX()+1;i++) {
	if(h_nominal->GetBinContent(i) > 0 &&  hdev->GetBinContent(i) > 0){
	  yval.push_back((hdev->GetBinContent(i)) / h_nominal->GetBinContent(i));
	}
	else {
	  yval.push_back(0.);
	}
      ...
      const double alpha = 1 - 0.6827;
      TGraphAsymmErrors * g2 = new TGraphAsymmErrors(hdev);
	...
	double U =  (N==0) ?  ( ROOT::Math::gamma_quantile_c(alpha,N+1,1) ) :
	  ( ROOT::Math::gamma_quantile_c(alpha/2,N+1,1) );
	if ( N!=0 ) {
	  yval_up.push_back((U-N)/ h_nominal->GetBinContent(i) );
	  yval_down.push_back((N-L)/ h_nominal->GetBinContent(i) );
	}
	else {
	  yval_up.push_back(0.);
	  yval_down.push_back(0.);
	}
      }

      for (Int_t i=1;i<=hdev->GetNbinsX()+1;i++) {
	if(hdev->GetBinContent(i) > 0.){
	  hdev->SetBinContent(i, hdev->GetBinContent(i)/h_nominal->GetBinContent(i));
	  hdev->SetBinError(i, 0.);
	}
      }

      TGraphAsymmErrors * g3 = new TGraphAsymmErrors(hdev);
      for (int i = 0; i < g3->GetN(); ++i) {
	cout << i << " yup = " << yval_down.at(i) <<  " " << yval_up.at(i) << endl;
	g3->SetPointEYlow(i,yval_down.at(i));
	g3->SetPointEYhigh(i,yval_up.at(i));
      }
...
void SetNomBinError(TH1* hnom, TH1* hup, TH1* hdown){
  ...
  for(int i=1; i < hnom->GetNbinsX()+1; i++){
    ...
    if(err1 > err2 ) hnom->SetBinError(i, 1.3* err1);
    if(err2 > err1 ) hnom->SetBinError(i, 1.3*err2);
  }
  return;
}
...
TH1* MakeErrorBand(TH1* hnom, TH1* hup, TH1* hdown){
  ...
  for(int i=1; i < errorband->GetNbinsX()+1; i++){
    ...
    errorband->SetBinError(i,1.3*bin_error);
  }
