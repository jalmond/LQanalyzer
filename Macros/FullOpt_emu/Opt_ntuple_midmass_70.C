...
void Opt_ntuple_midmass_70(){
  ...
  f_chain->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectronMuon_opt/HNEMu_SKnonprompt_dilep_5_3_14.root");
  f_chain->LoadTree(0) ;
  
  TChain * f_chainmc = new TChain("MyTree");
  f_chainmc->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectronMuon_opt/HNEMu_mc_5_3_14.root");
  f_chainmc->LoadTree(0) ;

  TChain * f_chainsig = new TChain("MyTree");
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectronMuon_opt/HNEMu_SKHNmue70_nocut_5_3_14.root");
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectronMuon_opt/HNEMu_SKHNemu70_nocut_5_3_14.root");
  TFile * file1 = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectronMuon_opt/HNEMu_SKHNmue70_nocut_5_3_14.root");
  TFile * file2 = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectronMuon_opt/HNEMu_SKHNemu70_nocut_5_3_14.root");
  ...
  f_chain->SetBranchAddress("emujj_mass_lowmass", &k_emujjmass);
  f_chain->SetBranchAddress("l2jj_mass_lowmass", &k_l2jjmass);
  f_chain->SetBranchAddress("jj_mass_lowmass", &k_jjmass);
  ...
  f_chainmc->SetBranchAddress("emujj_mass_lowmass", &k_emujjmass);
  f_chainmc->SetBranchAddress("l2jj_mass_lowmass", &k_l2jjmass);
  f_chainmc->SetBranchAddress("jj_mass_lowmass", &k_jjmass);
  ...
  f_chainsig->SetBranchAddress("emujj_mass_lowmass", &k_emujjmass);
  f_chainsig->SetBranchAddress("l2jj_mass_lowmass", &k_l2jjmass);
  f_chainsig->SetBranchAddress("jj_mass_lowmass", &k_jjmass);
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imemujjmin= 0; imemujjmin < iemujj_min_k; imemujjmin++){
      float memujjmin_cut = 80. + float(imemujjmin) * 10.;
      for(int imemujjmax= 0; imemujjmax <iemujj_max_k ; imemujjmax++){
	float memujjmax_cut = 140.+ float(imemujjmax)*5.;
	cout << memujjmin_cut <<   "  < M(emujj) < "<< memujjmax_cut << endl;
	for(int ipt1= 0; ipt1 <i_pt1_k ; ipt1++){
	  ...
	  for(int ipt2= 0; ipt2 < i_pt2_k ; ipt2++){
	    ...
	    for(int iemumin =0 ; iemumin < iemu_min_k; iemumin++){
	      float emumin_cut =  10. + float(iemumin)* 10.;
	      if(iemumin == 7) emumin_cut = 100.;
		
	      for(int iemumax =0 ; iemumax <iemu_max_k ; iemumax++){
		float emumax_cut =  60. + float(iemumax) * 5. ;
		...
		for(int il2jjmin =0 ; il2jjmin < i_ljj_min_k ; il2jjmin++){
		  float l2jjmin_cut = 0. + float(il2jjmin)* 10.;
			...
			float l2jjmax_cut =  120. + float(il2jjmax)* 5.;
