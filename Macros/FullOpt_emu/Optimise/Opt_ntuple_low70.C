...
void Opt_ntuple_low70(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee70_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee70_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 7; imeejjmin < 12 ; imeejjmin++){ 
      float meejjmin_cut = float(imeejjmin) * 10.;
      for(int imeejjmax= 16; imeejjmax < 23. ; imeejjmax++){ 
	float meejjmax_cut = 2000.;
