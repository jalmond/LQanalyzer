...
void Opt_ntuple_clever(){
  ...
  float eejjmin_low(0.),eejjmin_high(0.);
  for(int j=0; j < 200; j ++){
    float eejjmin= float(j)*10.;
    float cut_sum_sig_check=0.;
    for(Int_t i = 0; i < f_chainsig->GetEntries(); i++){
      f_chainsig->GetEntry(i) ;
      
      if(k_eejjmass < eejjmin) continue; 
      cut_sum_sig_check+=k_weight;
    }
    cout << eejjmin << " :" <<  cut_sum_sig_check << " " << sum_sigevent << " : " << cut_sum_sig_check/sum_sigevent << endl; 
    if(cut_sum_sig_check/sum_sigevent == 1.) eejjmin_low = eejjmin;
    if(cut_sum_sig_check/sum_sigevent > 0.5) eejjmin_high = eejjmin;
  }
  
  float eejjmax_low(0.),eejjmax_high(0.);
  for(int j=0; j < 200; j++){
    float eejjmax= 2000. - float(j)*10.;
    float cut_sum_sig_check=0.;
    for(Int_t i = 0; i < f_chainsig->GetEntries(); i++){
      f_chainsig->GetEntry(i) ;

      if(k_eejjmass > eejjmax) continue;
      cut_sum_sig_check+=k_weight;
    }
    if(cut_sum_sig_check/sum_sigevent > 0.95) eejjmax_high = eejjmax;
    if(cut_sum_sig_check/sum_sigevent > 0.5) eejjmax_low = eejjmax;
  }
  
  cout << eejjmin_low <<  " <  eejjmin < " << eejjmin_high << endl;;
  cout << eejjmax_low <<  " <  eejjmax < " << eejjmax_high << endl;;
  if(eejjmax_high > 500) eejjmax_low = eejjmax_high; 
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 0; imeejjmin < ((eejjmin_high - eejjmin_low)/10.) ; imeejjmin++){
      float meejjmin_cut =  eejjmin_low +float(imeejjmin) * 10.;
      for(int imeejjmax= 0; imeejjmax < ((eejjmax_high - eejjmax_low)/10.) ; imeejjmax++){
	float meejjmax_cut = eejjmin_high - (float(imeejjmax)*10); 
	if(imeejjmax == 0) meejjmax_cut = 2000.;
	else if(imeejjmax < 5){
	  meejjmax_cut = 550. - (float(imeejjmax)  * 50.);
	}
	else {
	  meejjmax_cut = 400 - (float(imeejjmax)  * 10.);
 	}
	...
	for(int ipt1= 0; ipt1 < 1. ; ipt1++){
	  ...
	  for(int ipt2= 0; ipt2 < 1. ; ipt2++){
	    ...
	    for(int ieemin =0 ; ieemin < 3 ; ieemin++){
	      float eemin_cut =  10. + float(ieemin)* 10.;
	      
	      for(int ieemax =0 ; ieemax < 5 ; ieemax++){
		float eemax_cut =  50. + float(ieemax)* 10.;
		if(ieemax==4) eemax_cut = 100000.;
		
		for(int ie2jjmax =0 ; ie2jjmax < 9 ; ie2jjmax++){
		  float e2jjmax_cut =  120. + float(ie2jjmax)* 10.;
		  
		  for(int istmin =0 ; istmin < 1 ; istmin++){
		    float stmin_cut = 0;
		    for(int istmax =0 ; istmax < 8 ; istmax++){
		      float stmax_cut = 180 + float(istmax) * 10.;
		      
		      for(int ijpt =0 ; ijpt < 2 ; ijpt++){
  ...
  cout << "Opt cut : " << 40 << " < M(e2jj) <  " << ime2jjmax_opt << endl;
