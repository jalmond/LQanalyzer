...
void MakeZPeakPlot(TString path) {
  ...
  if(fcf->IsZombie() || fdata->IsZombie()){
    cout << "No MC file" << endl;
    return;
  }
  vector<TString> hists;
  hists.push_back("h_eemass");
  hists.push_back("h_leadingElectronPt");
  hists.push_back("h_electrons_eta");
  for(vector<TString>::iterator it = hists.begin(); it!= hists.end(); it++){
    ...
    TH1F* hcf =  (TH1F*)fcf->Get(("SSee_Z/" + *it + "_SSee_Z").Data());
    TH1F* hdata =  (TH1F*)fdata->Get(("SSee_Z/" + *it + "_SSee_Z").Data());
    
    if(!hcf) cout << hcf->GetName() << " not found" << endl;
    if(!hdata) cout << hdata->GetName() << " not found" << endl;
    ...
    c1->SaveAs(("/home/jalmond/WebPlots/CF/ZPeak" + *it +".pdf").Data());
  }
