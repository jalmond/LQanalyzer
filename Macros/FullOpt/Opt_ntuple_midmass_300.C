...
void Opt_ntuple_midmass_300(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee300_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee300_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 35; imeejjmin < 36. ; imeejjmin++){
	...
	for(int ipt1= 0; ipt1 <20 ; ipt1++){
	  ...
	  for(int ipt2= 0; ipt2 < 1. ; ipt2++){
		...
		for(int ie1jjmin =8 ; ie1jjmin < 9 ; ie1jjmin++){
			...
			float e2jjmax_cut =  40000. + float(ie2jjmax)* 10.;
