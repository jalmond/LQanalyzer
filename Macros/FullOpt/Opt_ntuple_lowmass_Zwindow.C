...
void Opt_ntuple_lowmass_Zwindow(){
  ...
  for(unsigned int iZl=0; iZl < 4; iZl++){
    float Zlower = 70. + float(iZl)* 5.;
    for(unsigned int iZu=0; iZu < 4; iZu++){
      float Zupper = 110. - float(iZl)* 5.;
	...
	for(int ipt1= 0; ipt1 <1. ; ipt1++){
	  ...
	  for(int ipt2= 0; ipt2 < 1. ; ipt2++){
	    ...
	    for(int ieemin =0 ; ieemin < 1; ieemin++){
	      ...
	      for(int ieemax =0 ; ieemax <4 ; ieemax++){
		...
		if(ieemax == 3) eemax_cut = 100000.;
			...
			for(Int_t i = 0; i < f_chain->GetEntries(); i++){
			  f_chain->GetEntry(i) ;
			  if(k_eemass > Zlower && k_eemass < Zupper) continue;
  ...
  cout << "Opt cut : " << iZlow << " < M(Z) < " << iZup<< endl;
