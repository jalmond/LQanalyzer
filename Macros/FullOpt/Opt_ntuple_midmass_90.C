...
void Opt_ntuple_midmass_90(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee90_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee90_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 8; imeejjmin < 15. ; imeejjmin++){
      float meejjmin_cut = float(imeejjmin) * 10.;
      for(int imeejjmax= 0; imeejjmax <20. ; imeejjmax++){
	float meejjmax_cut = 150.+ float(imeejjmax)*10.;
