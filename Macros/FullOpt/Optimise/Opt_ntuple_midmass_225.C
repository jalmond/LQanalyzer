...
void Opt_ntuple_midmass_225(){
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 25; imeejjmin < 30. ; imeejjmin++){
	...
	for(int ipt1= 9; ipt1 <25. ; ipt1++){
	  float pt1_cut =  20. + float(ipt1)*5.;
	  for(int ipt2= 0; ipt2 < 8. ; ipt2++){
			...
			for(int ihtmin =0 ; ihtmin < 1 ; ihtmin++){
			  ...
			  for(int ihtmax =16 ; ihtmax < 17 ; ihtmax++){
				...
				for(int ijpt =4 ; ijpt <5 ; ijpt++){
				  ...
				  for(int injet = 15 ; injet<16 ; injet ++){
