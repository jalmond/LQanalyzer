...
void Opt_ntuple_midmass_275(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee275_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee275_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 27; imeejjmin < 35. ; imeejjmin++){
	...
	for(int ipt1= 8; ipt1 < 17. ; ipt1++){
	  float pt1_cut =  20. + float(ipt1)*5.;
	  for(int ipt2= 5; ipt2 < 9. ; ipt2++){
