...
void Opt_d0cut(){
  ...
  TH1F* h_npcutflow= (TH1F*)fnp->Get(("ISO_EEEB_cutflow"));
  TH1F* h_cfcutflow= (TH1F*)fcf->Get(("ISO_EEEB_cutflow"));
  TH1F* h_mccutflow= (TH1F*)fmc->Get(("ISO_EEEB_cutflow"));
  ...
  for(unsigned int im=0; im < masses.size(); im++){
    ...
    TH1F* h_cutflow= (TH1F*)fsig->Get(("ISO_EEEB_cutflow"));
    ...
    TH1F* h_mass_d0opt = new TH1F(("d0cut_opt_HN"+ masses.at(im)).Data(),("d0cut_opt_HN"+ masses.at(im)).Data(), 49,0.,49.);
    ...
    h_mass_d0opt->GetXaxis()->SetBinLabel(24,"dr03_B090_E050");
    h_mass_d0opt->GetXaxis()->SetBinLabel(18,"dr03_B100_E050");

    for(int i= 1; i < h_mass_d0opt->GetNbinsX()+1 ;i++){
    ...
    h_mass_d0opt->GetXaxis()->SetTitle("Reliso cut");
    ...
    h_mass_d0opt->GetYaxis()->SetRangeUser(0.001, 0.15);
  ...
  legend->Draw("same");
  c1->SaveAs(("/home/jalmond/WebPlots/Opt/iso_eb_optimise.pdf"));
}
