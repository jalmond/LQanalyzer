...
void Opt_ntuple_midmass_125(){
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 8; imeejjmin < 20. ; imeejjmin++){
      float meejjmin_cut = float(imeejjmin) * 10.;
      for(int imeejjmax= 10; imeejjmax <11. ; imeejjmax++){
	float meejjmax_cut = float(imeejjmax)*2000.;
	cout << meejjmin_cut <<   "  < M(eejj) < "<< meejjmax_cut << endl;
	for(int ipt1= 3; ipt1 <5 ; ipt1++){
