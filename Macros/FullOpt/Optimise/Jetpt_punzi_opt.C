...
//void MET_punzii_opt(){
{
  ...
  TString  masses[imasses] = {"40", "60", "80",  "100",  "125", "150",  "200", "300", "400", "500"};
  ...
  float pt1cut[imasses] = { 20., 20., 20.,20.,20.,40.,60.,120.,120.,120.};
  float pt2cut[imasses] = { 15., 15., 15.,15.,15.,25.,25.,25.,25.,25.};
  float eejjcut[imasses] = { 80., 80., 80.,110., 120., 170., 230., 350., 350., 350.};
  ...
  for(int im = 0 ; im < imasses ; im++){
    ...
    cout << "mass = " << masses[im] << endl;
    float  k_met=0., k_eemass=0., k_eejjmass=0., k_e1jjmass=0., k_e2jjmass=0.,  k_st=0., k_ht=0;
    float k_weight=0.;
    int k_njet;
    ...
    const int nm = 20;
    float xaxis[nm];
    for(int met =0; met <nm; met ++){
      float metcut = 20. + float(met)*5.;
      xaxis[met] = metcut;
    }
    ...
    for(unsigned int imet=0; imet < nm; imet++){
      float jetpt_cut = xaxis[imet];
      cout << "Jetpt : "<< jetpt_cut << endl;
	...
	if(k_met > 30) continue;
	if(k_j1pt < jetpt_cut) continue;
	if(k_el1pt < pt1cut[im]) continue;
	if(k_el2pt < pt2cut[im]) continue;
	if(k_eejjmass < eejjcut[im]) continue; 
	cut_sum+=k_weight;
	np_sum+=k_weight;
      }
    ...
    //cout << "Opt cut : Lead Jet >  " << xaxis[imet_opt] << endl;
  ...
  TH1 *frame = new TH1F("frame","",1000,20,100);
  ...
  frame->GetYaxis()->SetRangeUser(0.0001,1.);
  frame->Draw(" ");
  c1->SetLogy();
  ...
  c1->SaveAs("/home/jalmond/WebPlots/Opt/Punzi/JetPt_punzi_optimisation.pdf");
}
