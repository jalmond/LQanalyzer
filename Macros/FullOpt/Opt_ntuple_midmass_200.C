...
void Opt_ntuple_midmass_200(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_basic_SKHNee200_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_basic_SKHNee200_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 0; imeejjmin <7 ; imeejjmin++){
      float meejjmin_cut = 220. + float(imeejjmin) * 10.;
	  ...
	  for(int ipt2= 0; ipt2 < 1. ; ipt2++){
		...
		for(int ieemax =0 ; ieemax <1 ; ieemax++){
		  ...
		  for(int ie1jjmin =0 ; ie1jjmin < 1 ; ie1jjmin++){
			...
			float e2jjmin_cut = 160. + float(ie2jjmin)* 10.;
			
			for(int ie2jjmax =0; ie2jjmax < 1 ; ie2jjmax++){
			  float e2jjmax_cut = 255. + float(ie2jjmax)* 5.;
