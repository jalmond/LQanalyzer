...
void Opt_ntuple_midmass_400(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_basic_SKHNee400_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_basic_SKHNee400_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 0; imeejjmin <1 ; imeejjmin++){
      float meejjmin_cut = 360. + float(imeejjmin) * 10.;
	...
	for(int ipt1= 0; ipt1 <1. ; ipt1++){
	  float pt1_cut =  120. + float(ipt1)*5.;
