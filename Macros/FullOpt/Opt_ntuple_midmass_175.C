...
void Opt_ntuple_midmass_175(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_basic_SKHNee175_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_basic_SKHNee175_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 0; imeejjmin <10 ; imeejjmin++){
      float meejjmin_cut = 220. + float(imeejjmin) * 5.;
	  ...
	  for(int ipt2= 0; ipt2 < 4. ; ipt2++){
		...
		for(int ieemax =0 ; ieemax <1 ; ieemax++){
		  ...
		  for(int ie1jjmin =0 ; ie1jjmin < 1 ; ie1jjmin++){
			...
			float e2jjmin_cut = 140. + float(ie2jjmin)* 5.;
