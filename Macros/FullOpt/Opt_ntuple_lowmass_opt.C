...
void Opt_ntuple_lowmass_opt(){
  ...
  float osig_eff = sum_sigevent/nsig ;
  float ototal_bkg = sum_event;
  float obkgtmp = ototal_bkg + (0.35* onp_sum)* (0.35* onp_sum);
  float odenom = 1. + sqrt(obkgtmp);
  float opunzi = osig_eff / odenom;

  cout << "Original values: " <<  endl;
  cout << "Punzi = " << opunzi << "  sig eff = " << osig_eff << "  nbkg = " << ototal_bkg << endl;
  

  for(unsigned int imet=0; imet < METcut.size(); imet++){
    float met_cut = METcut.at(imet);
    for(int imeejjmin= 0; imeejjmin < 4. ; imeejjmin++){
