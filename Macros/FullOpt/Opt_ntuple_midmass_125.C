...
void Opt_ntuple_midmass_125(){
  ...
  f_chainsig->Add("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee125_nocut_5_3_14.root");
  TFile * file = new TFile("/home/jalmond/Analysis/LQanalyzer/data/output/SSElectron_ntup/HNDiElectron_SKHNee125_nocut_5_3_14.root");
  cout << file << endl;
  ...
  for(unsigned int imet=0; imet < METcut.size(); imet++){
    ...
    for(int imeejjmin= 14; imeejjmin < 15. ; imeejjmin++){
	...
	for(int ipt1= 0; ipt1 <1 ; ipt1++){
	  ...
	  for(int ipt2= 0; ipt2 < 1. ; ipt2++){
	    ...
	    for(int ieemin =0 ; ieemin < 1 ; ieemin++){
	      float eemin_cut =  20. + float(ieemin)* 5.;
		...
		for(int ie1jjmin =8 ; ie1jjmin < 9 ; ie1jjmin++){
		    ...
		    for(int ie2jjmin =0 ; ie2jjmin < 20 ; ie2jjmin++){
		      float e2jjmin_cut = 80. + float(ie2jjmin)* 5.;
